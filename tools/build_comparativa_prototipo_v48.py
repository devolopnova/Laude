#!/usr/bin/env python3
"""
build_comparativa_prototipo_v48.py

Evolucion sobre v47 (ver tools/build_comparativa_prototipo_v47.py) --
implementa UNICAMENTE el "FILTRO POR EDAD" del selector movil acordado
con el usuario (18-ago-2026): un `<select>` "Filtrar por edad" dentro
del panel "Selecciona una sillita", justo debajo del buscador de texto
-- NO es un segundo buscador, filtra el MISMO listado y se combina
(acumulativo) con el texto ya escrito en el buscador.

Piezas nuevas:
  - AGE_FILTER_OPTIONS: las 12 franjas pedidas (0..10 años + "10+
    años"). No son datos del dataset -- son las franjas del propio
    filtro.
  - build_mobile_product_payload(p) ahora incluye edadMinMeses/
    edadMaxMeses, calculados con parse_edad_rango_meses() -- la MISMA
    funcion que ya usa la tabla de PC para ordenar por Edad, ninguna
    logica de rango nueva. null cuando el campo Edad no tiene un rango
    reconocible (igual que en PC).
  - build_mobile_selector() añade el `<label>`/`<select
    id="mc-selector-age">` dentro de `.mc-selector-panel-inner`, entre
    el buscador y el listado de resultados.
  - JS: `ageFilter` (estado, '' = sin filtro), `passesAgeFilter(p)`
    (compara edadMinMeses/edadMaxMeses del producto contra la edad
    elegida -- para "10plus" comprueba edadMaxMeses >= 120, para el
    resto comprueba que la edad elegida caiga dentro del rango).
    renderResults() ahora exige TAMBIEN passesAgeFilter(p) ademas de la
    coincidencia de texto -- mismo listado, filtros acumulativos. Sin
    resultados por edad se muestra "No hemos encontrado sillitas para
    esta edad." (en vez del mensaje generico). El filtro se resetea a
    "Selecciona la edad" cada vez que se abre/cierra el panel (openPanel/
    closePanel), igual que ya se reseteaba el texto del buscador.
  - Un producto sin rango de edad reconocible queda EXCLUIDO cuando hay
    un filtro de edad activo -- nunca se afirma una compatibilidad que
    no consta en el dato real (con el dataset actual esto no ocurre:
    los 30 productos tienen Edad reconocible).

El filtro funciona igual para Sillita 1 y Sillita 2 (no depende de
`pickingSlot`, actua sobre el listado que ya se muestre) y no toca en
absoluto la logica de asignacion de slot ni la exclusion del producto
ya elegido en el otro slot (sigue en renderResults(), sin cambios).

No se toca: diseño de las tarjetas de resultado (imagen/iniciales,
marca, modelo, rango, valoración, valoraciones, precio, ISOFIX -- sin
cambios), la tabla de PC, el dataset, la matriz de caracteristicas, la
ficha tecnica, la tarjeta VS, las imagenes, la logica de comparacion,
las ordenaciones, ni ninguna otra funcionalidad del selector salvo lo
estrictamente necesario para integrar el filtro.

Uso:
    python tools/build_comparativa_prototipo_v48.py
"""
...
OUT = "prototipo-comparativa-sillas-v48.html"
...
<title>PROTOTIPO v48 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v48 — <b>no es la web real</b> — misma fuente que v4-v47: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Nuevo filtro por edad (18-ago-2026): en el panel "Selecciona una sillita", debajo del buscador, un desplegable <b>"Filtrar por edad"</b> (0–10 años + 10+) que se combina con el texto de búsqueda sobre el mismo listado — no es un segundo buscador. Usa el rango real de Edad de cada producto (misma lógica que ya ordena la tabla PC). Redimensiona a &lt;768px para probarlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v48.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
