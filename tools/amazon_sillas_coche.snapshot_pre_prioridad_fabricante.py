...
def resolve_grupo_r44(raw: Dict, fab_entry: Optional[Dict], asin: str) -> Dict:
    """Grupo (clasificacion 0/0+/1/2/3), auditado y aprobado por el usuario
    el 2026-08-11. Pese al nombre historico del campo (grupo_r44), este
    atributo es INDEPENDIENTE de la homologacion: representa la etiqueta
    comercial de "Grupo X" que muchas fichas -- incluidas las de sillas
    R129/i-Size -- siguen usando de forma informal para describir las
    etapas de uso del producto. No confundir con `normativa` (R129/i-Size
    vs R44), que se resuelve por separado en resolve_normativa y no cambia
    con esta funcion. Nunca se infiere un grupo a partir de edad/peso/
    altura/normativa/tipo de silla -- solo evidencia textual explicita de
    "Grupo X"/"Group X" (Amazon y/o fabricante).

    Estados posibles (nunca "Si"/"No"/"No determinado" para este atributo,
    no es booleano):
      - Valor concreto ('0', '0+', '1', '2/3', '1/2/3', '0+/1/2/3', etc.):
        evidencia explicita y clara.
      - "Revisar": Amazon y fabricante mencionan grupos distintos.
      - "Sin especificar": ninguna fuente disponible menciona un grupo
        explicito tras la auditoria.

    Confianza: "alta" cuando Amazon y fabricante coinciden o cuando solo el
    fabricante lo confirma; "media" cuando solo Amazon lo confirma (fuente
    unica) o cuando no hay evidencia y el fabricante no esta verificado
    para este ASIN; "alta" cuando no hay evidencia pero SI se pudo auditar
    tambien la ficha oficial (identidad verificada) sin encontrar nada."""
    ...
    if amazon_valor is not None and fab_valor is not None and fab_valor != amazon_valor:
        ...
        return campo(origen, None, "Revisar", fuente="amazon_y_fabricante",
