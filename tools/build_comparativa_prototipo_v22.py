#!/usr/bin/env python3
"""
build_comparativa_prototipo_v22.py

Evolucion sobre v21 (ver tools/build_comparativa_prototipo_v21.py) --
implementa UNICAMENTE el punto 7.6 acordado con el usuario (16-ago-2026):
ISOFIX y 360º pasan de simbolos (✓/✕/—) a texto explicito ("Sí"/"No"/
"Sin especificar"), mismo tamaño base (12.5px) que Normativa/
Orientación/Arnés, sin negrita, sin color por valor (texto plano
var(--ink); "Sin especificar" en el mismo gris discreto de siempre,
#9C9A93). Sin iconos, sin badges, sin decoracion.

Es un cambio PURAMENTE de presentacion: la funcion valor_badge() sigue
decidiendo Sí/No/Sin especificar exactamente con la misma condicion que
antes decidia ✓/✕/— (v.startswith("Sí")/"No" / es_sin_especificar(v)),
solo cambia el texto que se pinta. Los data-sort-isofix-cat/
sort-giro360-cat (usados por la ordenacion de caracteristicas) no se
tocan -- siguen calculandose en Python a partir del mismo dato origen,
asi que la ordenacion "ISOFIX/360º: Sí u No primero" sigue funcionando
identica.

UNICO ajuste no puramente tipografico: col-isofix/col-360 pasan de 55
a 76px, con su padding horizontal reducido de 12px a 4px. Necesario
porque "especificar" mide ~66px a 12.5px (medido con
canvas.measureText) y no cabia en el ancho/padding anteriores -- sin
este ajuste el texto se habria desbordado o habria tenido que
truncarse/partirse a media palabra. No aumenta la altura de fila:
"Rango de uso" ya es mas alto que estas 2 lineas en casi todas las
filas.

No se modifica ningun dato del dataset, ningun JSON, ninguna otra
columna, ficha tecnica, boton "Ver en Amazon", la ordenacion, el
selector "Ordenar por", ni la cabecera del punto 7.1. No se toca
amazon_import.py ni ningun archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v22.py
"""
...
OUT = "prototipo-comparativa-sillas-v22.html"
...
def valor_badge(valor):
    ...
    "Sin especificar" en vez de simbolos ✓/✕/— -- solo cambia la
    presentacion, el dato origen (v.startswith("Sí")/"No") es el mismo
    que ya se usaba para decidir el simbolo antes."""
    v = valor.strip()
...
# pasan de 55 a 76px en v22 (punto 7.6): "especificar" mide ~66px a
# 12.5px (medido con canvas.measureText) y no cabe en los ~31px de area
# util que quedaba con el ancho/padding anteriores -- el aumento es lo
# minimo necesario para que "Sin especificar" quepa en 2 lineas
# ("Sin"/"especificar") sin desbordar ni partir la palabra. No aumenta
# la altura de fila: "Rango de uso" ya es mas alto que estas 2 lineas
# en casi todas las filas. El resto de columnas mantiene su ancho.
COLUMN_WIDTHS = [250, 85, 84, 90, 190, 76, 120, 76, 150, 160]
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v22 — Comparativa sillitas de coche</title>
...
/* ISOFIX/360º -- punto 7.6 (16-ago-2026): texto explicito "Sí"/"No"/
   "Sin especificar" en vez de simbolos ✓/✕/—. Mismo tamaño base
   (12.5px) que Normativa/Orientación/Arnés, texto plano sin negrita ni
   color -- "Sin especificar" usa el mismo gris discreto que el resto
   de la tabla. Sin iconos, sin badges. Padding horizontal reducido
   (12px -> 4px) SOLO en estas 2 columnas -- son columnas de estado
   compacto, no de texto largo, y ese espacio extra es lo que permite
   que "Sin especificar" quepa en el nuevo ancho (76px) sin desbordar. */
td.col-isofix,td.col-360{{text-align:center;max-width:76px;white-space:normal;padding-left:4px;padding-right:4px;}}
...
<div class="proto-banner">⚠ Prototipo v22 — <b>no es la web real</b> — misma fuente que v4-v21: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Cambio sobre v21 (punto 7.6, 16-ago-2026): ISOFIX y 360º pasan de símbolos (✓/✕/—) a texto explícito — <b>Sí / No / Sin especificar</b> — mismo tamaño base (12.5px) y estilo plano que Normativa/Orientación/Arnés, sin iconos ni badges. Es solo presentación: el dato y la lógica de ordenación por ISOFIX/360º no cambian. Único ajuste no tipográfico: estas 2 columnas pasan de 55 a 76px (con menos padding interno) porque "especificar" no cabía en el ancho anterior sin desbordar ni partirse a media palabra.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v22.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v22] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}} | cabeceras ordenables: ${{sortableHeaders}} | items en menus de cabecera: ${{totalMenuItems}} | opciones select: ${{selectOptions}}`);
