#!/usr/bin/env python3
"""
build_comparativa_prototipo_v26.py

Evolucion sobre v25 (ver tools/build_comparativa_prototipo_v25.py) --
ajuste estetico UNICO (16-ago-2026): elimina el subrayado del texto de
los botones "Ver en Amazon →", tanto el de la tabla principal
(.btn-amazon-row) como el de la ficha tecnica (.btn-amazon).

Causa: .btn-amazon-row ya tenia `text-decoration:none` en reposo, pero
.btn-amazon (el de la ficha tecnica) no lo tenia en absoluto -- al ser
un <a>, heredaba el subrayado por defecto del navegador. Se añade
`text-decoration:none` a ambas reglas base, y tambien a sus `:hover`
(por si el subrayado solo aparecia al pasar el raton), para que los dos
botones tengan exactamente el mismo tratamiento.

No se toca nada mas: mismo color, fondo, tamaño, tipografia, padding,
bordes y comportamiento de hover (solo cambia el color de fondo al
pasar el raton, como ya hacia). No se modifica ningun dato del
dataset, ningun JSON, la tabla principal, ninguna columna, la
cabecera, la ordenacion, la estructura de la ficha tecnica, ni los
enlaces/ASIN de los botones. No se toca amazon_import.py ni ningun
archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v26.py
"""
...
OUT = "prototipo-comparativa-sillas-v26.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v26 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v26 — <b>no es la web real</b> — misma fuente que v4-v25: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo el subrayado de "Ver en Amazon")</div>
<p class="proto-note">
  Ajuste sobre v25 (16-ago-2026): se elimina el subrayado del texto de los botones <b>"Ver en Amazon →"</b>, tanto el de la tabla principal como el de la ficha técnica (el segundo no tenía <code>text-decoration:none</code> y heredaba el subrayado por defecto del navegador). Mismo color, fondo, tamaño, tipografía, padding y hover de siempre. No se ha tocado ningún otro elemento, dato, enlace ni ASIN.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v26.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
