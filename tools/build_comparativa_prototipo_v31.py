#!/usr/bin/env python3
"""
build_comparativa_prototipo_v31.py

Evolucion sobre v30 (ver tools/build_comparativa_prototipo_v30.py) --
implementa UNICAMENTE el Ajuste 7.16 acordado con el usuario
(18-ago-2026), con dos cambios:

  1. Centrado vertical global en la tabla: Normativa, ISOFIX,
     Orientación, 360º y Arnés (Rango de uso y Acciones ya estaban
     correctos desde antes, ver punto 7.11) mostraban una asimetria
     real de 2-5px entre el hueco superior e inferior de su contenido
     dentro de la celda (medido con getBoundingClientRect), pese a
     heredar `vertical-align:middle` de `tbody td` -- causada por las
     metricas de line-height/fuente de un nodo de texto/inline-block
     suelto directamente en el <td>. Se corrige envolviendo el
     contenido en un elemento `display:block` (`.cell-block` para
     Normativa/Orientación/Arnés via celda_principal(), y cambiando
     `.txt-plain` de `inline-block` a `block` para ISOFIX/360º), el
     mismo patron que ya usaban Rango de uso (.rango-block) y Acciones
     (.action-group). No cambia tamaños, pesos, colores, margenes ni
     anchos de columna -- solo el layout interno para el centrado.
  2. Ordenacion de "Valoración" ampliada: ahora ordena primero por
     puntuacion (mayor primero en descendente) y, en caso de empate,
     por nº de valoraciones (siempre mayor primero, en los dos
     sentidos -- mismo patron de desempate fijo que ya usaba
     "grupo-asc/desc"). El icono de esta cabecera pasa a un ciclo
     EXCLUSIVO de 3 estados (clic 1: descendente, clic 2: ascendente,
     clic 3: sin ordenar/orden original) -- el resto de cabeceras
     (Precio/ISOFIX/360º) conservan su ciclo de 2 estados de siempre,
     sin cambios.
  3. Ajuste adicional pedido por el usuario en vivo durante esta misma
     revision (18-ago-2026): varias columnas de datos no quedaban
     centradas horizontalmente pese a que su cabecera SI estaba
     centrada (thead th.col-price/col-normativa/col-orientacion/
     col-anclaje ya llevaban text-align:center desde el punto 7.1) --
     el <td> correspondiente heredaba el text-align:left por defecto
     del navegador al no tener su propia regla. Se añade
     `text-align:center` a td.col-rango, td.col-price, td.col-normativa,
     td.col-orientacion y td.col-anclaje -- no cambia tamaños, pesos,
     colores ni anchos, solo la alineacion horizontal de esas columnas.
     (Producto se queda a la izquierda a proposito, igual que su
     cabecera; Valoración/ISOFIX/360º/Acciones ya estaban centradas.)

No se modifica ningun dato del dataset, ningun JSON, ninguna otra
columna (Producto/Precio/Normativa/Rango de uso/ISOFIX/Orientación/
360º/Arnés/Acciones), la ficha tecnica, los botones Amazon, los
enlaces/ASIN, los estilos generales/colores/anchos de columna, el
comportamiento responsive, ni el resto de los 25 criterios de
ordenacion. No se toca amazon_import.py ni ningun archivo de
produccion.

Uso:
    python tools/build_comparativa_prototipo_v31.py
"""
...
OUT = "prototipo-comparativa-sillas-v31.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v31 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v31 — <b>no es la web real</b> — misma fuente que v4-v30: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajuste 7.16 (18-ago-2026): <b>centrado vertical</b> corregido en Normativa/ISOFIX/Orientación/360º/Arnés (asimetría de píxeles por metricas de fuente, ahora envueltos en bloques). La cabecera <b>Valoración</b> ordena ahora por puntuación y, en empate, por nº de valoraciones (siempre mayor primero); su icono pasa a un ciclo exclusivo de 3 estados: descendente → ascendente → sin ordenar.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v31.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function setupSort() {{
  ...
  // Nº de valoraciones, Peso -- Peso SIEMPRE usa el maximo, en los dos
  // sentidos, tal como pidio el usuario). Valoracion tiene su propia
  // rama dedicada mas abajo (desempate por nº de valoraciones), no va
  // aqui.
  ...
  function compareRows(a, b, key) {{
    ...
    if (key === 'valoracion-desc' || key === 'valoracion-asc') {{
      const dir = key === 'valoracion-desc' ? -1 : 1;
      const va = getNum(a, 'sortValoracion'), vb = getNum(b, 'sortValoracion');
      // "Sin especificar" siempre al final, en los dos sentidos.
      if (va === null && vb === null) return origA - origB;
      if (va === null) return 1;
      if (vb === null) return -1;
      if (va !== vb) return dir * (va - vb);
      // Empate en puntuacion -> desempate por nº de valoraciones, SIEMPRE
      // de mayor a menor (regla fija, no depende del sentido elegido --
      // igual patron que el desempate de "grupo-asc/desc" de arriba).
      const na = getNum(a, 'sortNval'), nb = getNum(b, 'sortNval');
      if (na === null && nb === null) return origA - origB;
      if (na === null) return 1;
      if (nb === null) return -1;
      if (na !== nb) return -1 * (na - nb);
      return origA - origB;
    }}
  ...
  // Valoración (Ajuste 7.16, 18-ago-2026): ciclo de 3 estados, exclusivo
  // de esta cabecera -- clic 1: descendente (mayor puntuacion primero,
  // desempate por nº de valoraciones), clic 2: ascendente, clic 3: sin
  // ordenar (vuelve al orden original). El resto de cabeceras conservan
  // su ciclo de 2 estados de siempre (wireToggle), sin cambios.
  document.querySelector('[data-sort-toggle="valoracion"]').addEventListener('click', () => {{
    const next = currentSortKey === 'valoracion-desc' ? 'valoracion-asc'
      : currentSortKey === 'valoracion-asc' ? 'default'
      : 'valoracion-desc';
    triggerSort(next);
  }});
