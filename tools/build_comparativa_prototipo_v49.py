#!/usr/bin/env python3
"""
build_comparativa_prototipo_v49.py

Evolucion sobre v48 (ver tools/build_comparativa_prototipo_v48.py) --
CORRECCION acordada con el usuario (18-ago-2026): "Buscar por marca o
modelo" y "Filtrar por edad" deben estar SIEMPRE disponibles e
independientes dentro del panel "Selecciona una sillita", sin que el
panel recuerde un "modo" que bloquee el uso del otro.

Causa raiz: habia DOS sitios distintos que abrian el panel --
openPanel() (usado al tocar/activar un slot) y una rama de apertura
propia dentro de handleTopSearch() (usada por el buscador general, de
fuera de los slots). Solo openPanel() reseteaba el filtro de edad
(`ageFilter` + el `<select>`); la rama de handleTopSearch() abria el
panel sin tocar ninguno de los dos, asi que un filtro de edad quedaba
"pegado" si la siguiente apertura del panel venia por el buscador
general.

Cambio: se extrae resetPanelState(slotKey) -- un UNICO sitio que
resetea pickingSlot/pendingAsin/ageFilter/el `<select>` de edad/el
texto del buscador del panel -- y ahora lo usan las 2 rutas de apertura
por igual (openPanel() y la rama de apertura de handleTopSearch()), asi
que el panel siempre arranca limpio venga por donde venga. closePanel()
tambien pasa a usar resetPanelState(null), asi que cerrar el panel deja
el mismo estado limpio que abrirlo de nuevo.

Se mantiene la diferencia de foco que ya existia a proposito: openPanel()
sigue haciendo focus() en el buscador del panel (apertura por toque de
slot); la apertura desde el buscador general NO roba el foco (el
usuario sigue escribiendo ahi mismo).

No se toca: la logica de asignacion de slot (primera->Slot1, segunda->
Slot2, tocar un slot ya elegido lo activa para sustituirlo, nunca el
mismo ASIN en los 2 slots), el filtro de edad en si (passesAgeFilter,
AGE_FILTER_OPTIONS), el diseño de las tarjetas de resultado, la tabla
de PC, el dataset, la matriz de caracteristicas, la ficha tecnica, la
tarjeta VS, las imagenes, la logica de comparacion ni las ordenaciones.

Uso:
    python tools/build_comparativa_prototipo_v49.py
"""
...
OUT = "prototipo-comparativa-sillas-v49.html"
...
<title>PROTOTIPO v49 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v49 — <b>no es la web real</b> — misma fuente que v4-v48: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Corrección (18-ago-2026): "Buscar por marca o modelo" y "Filtrar por edad" quedan siempre disponibles e independientes — antes, un filtro de edad activo podía quedarse "pegado" al reabrir el panel desde el buscador general. Ahora las 2 rutas de apertura del panel resetean ambos filtros por igual. Redimensiona a &lt;768px para probarlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v49.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
