#!/usr/bin/env python3
"""
build_comparativa_prototipo_v7.py

Evolucion de presentacion sobre v6 (ver
tools/build_comparativa_prototipo_v6.py) -- MISMA fuente de datos, MISMO
diseno base. Cambio: se fusionan visualmente ALTURA + GRUPO + PESO en
una unica columna "Rango de uso" (antes Altura vivia en su propia
columna y Grupo+Peso en otra). Se elimina la columna independiente
"Altura".

Jerarquia dentro de "Rango de uso": Altura (dato principal, mayor peso
tipografico) -> Grupo -> Peso, cada uno SOLO SI EXISTE (si un campo esta
"Sin especificar" no se muestra esa linea -- nunca se rellena el hueco
con guion ni con el texto "Sin especificar", simplemente no aparece esa
linea). El campo "grupo" del dataset sigue sin reinterpretarse como
clasificacion R129 -- "Rango de uso" es una etiqueta editorial que junta
datos utiles, la normativa sigue en su propia columna aparte.
...
Uso:
    python tools/build_comparativa_prototipo_v7.py
"""
...
OUT = "prototipo-comparativa-sillas-v7.html"
...
def rango_uso_html(altura, grupo, peso):
    """Fusiona Altura + Grupo + Peso en una sola celda, en ese orden de
    jerarquia (altura = dato principal). Cada linea SOLO aparece si el
    dato existe -- nunca se rellena el hueco con guion ni con el texto
    'Sin especificar', simplemente no se genera esa linea."""
    lineas = []
    for valor_campo, css_cls in ((altura, "rango-altura"), (grupo, "rango-grupo"), (peso, "rango-peso")):
...
def build_row(p, idx):
    ...
    rango_html = rango_uso_html(altura, grupo, peso_recomendado)
...
# Se elimina la columna independiente "Altura" (fusionada dentro de
# "Rango de uso"). El hueco liberado (115px) se reparte dando mas ancho
# a "Rango de uso" (150 -> 190px, ahora lleva hasta 3 lineas) y el resto
# queda de margen -- la tabla resulta mas estrecha que en v6.
COLUMN_WIDTHS = [260, 95, 110, 190, 65, 65, 150, 150]
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v7 — Comparativa sillitas de coche</title>
...
/* "Rango de uso": Altura (dato principal) > Grupo > Peso, jerarquia
   tipografica descendente. Cada linea solo existe en el HTML si el dato
   esta presente (ver rango_uso_html en el script) -- aqui solo se
   define el estilo de cada nivel cuando aparece. */
td.col-rango{{white-space:normal;}}
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v7.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v7] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}}`);
