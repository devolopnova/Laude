#!/usr/bin/env python3
"""
build_comparativa_prototipo_v38.py

Evolucion sobre v37 (ver tools/build_comparativa_prototipo_v37.py) --
implementa UNICAMENTE la "FICHA TÉCNICA DEL COMPARADOR MÓVIL" acordada
con el usuario (18-ago-2026): seccion independiente y colapsable debajo
de la matriz principal ya aprobada (Precio/Normativa/Rango de uso/
ISOFIX/Orientación/360º/Arnés, sin tocar), con 5 campos EXCLUSIVOS que
no repiten nada de esa matriz:

    Reposacabezas, Peso de la silla, Protección lateral, Reclinable,
    Funda lavable.

Travel System queda fuera a peticion expresa (solo 4/30 productos con
dato, 13% de cobertura).

Misma estructura de grid de 3 columnas (CARACTERÍSTICA | SILLITA A |
SILLITA B) que la matriz principal -- de hecho reutiliza tal cual las
clases .mc-feat-label/.mc-feat-val, ninguna regla CSS duplicada.
build_mobile_ficha_values() calcula cada valor reutilizando
EXACTAMENTE los mismos campos/funciones que ya usa build_row() para la
tabla de PC (campo()/valor_badge()) -- ningun dato escrito a mano.

Estados Sí/No/Sin especificar: los 4 campos Sí-o-No (todos menos "Peso
de la silla", que es una medida) reutilizan valor_badge(), la MISMA
funcion que ya usa la tabla de PC para ISOFIX/360º -- ya distingue
exactamente los 3 estados pedidos sin inventar nada: "Sí" solo si el
dato lo confirma, "No" solo si el dato lo dice explicitamente (nunca se
sintetiza a partir de una ausencia), "Sin especificar" en gris discreto
(clase ya existente txt-plain--unk) cuando no hay dato. Comprobado
contra el dataset real: proteccion_lateral y funda_lavable NUNCA traen
un "No" explicito (solo "Sí" o ausente), asi que para esos 2 campos en
la practica solo apareceran los estados "Sí"/"Sin especificar" -- eso
es fiel al dato, no una limitacion de esta implementacion.

Interaccion: boton "Ficha técnica ▾" con su propia tarjeta (mismo
lenguaje visual que el resto -- borde/radio/sombra), panel colapsado
por defecto (atributo `hidden`), toggle en un IIFE nuevo
(setupMobileFicha()) con IDs/clases propios (mc-ficha-toggle/
mc-ficha-panel) -- cero relacion con el sistema de toggles de la ficha
tecnica de PC (.btn-toggle), sin ninguna interferencia.

No se toca la Fase 1 (tarjetas VS) ni la matriz principal ya aprobada
(build_mobile_features, MOBILE_FEATURE_ROWS). No se modifica la tabla
de PC, el dataset, las ordenaciones, el menu de Valoración, la ficha
tecnica de PC ni los botones/enlaces de Amazon.

Uso:
    python tools/build_comparativa_prototipo_v38.py
"""
...
OUT = "prototipo-comparativa-sillas-v38.html"
...
def build_mobile_features(productos_ab):
  ...
  <div class="mc-feat-val">{valores[0][attr]}</div>
  <div class="mc-feat-val">{valores[1][attr]}</div>''')
...
def build_mobile_compare(productos):
    """Construye el bloque VS de 2 productos para movil (tarjetas +
    matriz de caracteristicas), usando EXACTAMENTE el mismo dataset y
    los mismos helpers de formato (fmt_precio/fmt_valoracion/fmt_n_val/
    campo/valor_badge/rango_uso_html/...) que ya usa build_row() para
    la tabla de PC -- ningun dato nuevo, ninguna duplicacion manual."""
    por_asin = {p["asin"]: p for p in productos}
    productos_ab = [por_asin[asin] for asin in DEMO_COMPARE_ASINS]
    columnas = []
    for p in productos_ab:
        asin = p["asin"]
...
<title>PROTOTIPO v38 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v38 — <b>no es la web real</b> — misma fuente que v4-v37: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Responsive móvil (18-ago-2026): nueva sección colapsable <b>Ficha técnica</b> debajo de la matriz principal, con 5 campos exclusivos (Reposacabezas, Peso de la silla, Protección lateral, Reclinable, Funda lavable — sin Travel System, 13% de cobertura). Empieza cerrada; distingue Sí / No / Sin especificar sin inventar datos. Redimensiona a &lt;768px para verla.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v38.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
