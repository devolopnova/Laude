#!/usr/bin/env python3
"""
build_comparativa_prototipo_v47.py

Evolucion sobre v46 (ver tools/build_comparativa_prototipo_v46.py) --
implementa UNICAMENTE el cambio de color del precio acordado con el
usuario (18-ago-2026), EXCLUSIVO de movil: el precio de la tarjeta VS
superior pasa a usar el mismo color de acento que ya usa la marca
(CYBEX/MAXI-COSI, etc.), en vez del color de texto neutro.

Cambio (unico, CSS): `.mc-product-price` pasa de `color:var(--ink)` a
`color:var(--accent-deep)` -- la MISMA variable que ya usa
`.mc-product-brand`. Tamaño (16px), peso (700, negrita) y posicion no
cambian -- solo el color.

`.mc-product-price` es una clase EXCLUSIVA de la tarjeta VS movil (la
tabla de PC usa `.price-current`, una clase totalmente distinta, sin
tocar), asi que este cambio no puede afectar a escritorio.

Jerarquia resultante: Marca y Precio en color de acento; Modelo y
Valoración en color de texto principal; Nº de valoraciones en color
secundario -- exactamente como se pidio, sin tocar ningun otro
elemento de la tarjeta ni de la matriz/ficha tecnica.

Uso:
    python tools/build_comparativa_prototipo_v47.py
"""
...
OUT = "prototipo-comparativa-sillas-v47.html"
...
<title>PROTOTIPO v47 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v47 — <b>no es la web real</b> — misma fuente que v4-v46: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Cambio de color (18-ago-2026): en la tarjeta VS superior, el <b>precio</b> pasa a usar el mismo color de acento que la <b>marca</b> — mismo tamaño, negrita y posición de siempre. Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v47.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
