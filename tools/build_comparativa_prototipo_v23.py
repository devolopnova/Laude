#!/usr/bin/env python3
"""
build_comparativa_prototipo_v23.py

Evolucion sobre v22 (ver tools/build_comparativa_prototipo_v22.py).
v22 implemento el punto 7.6: ISOFIX y 360º pasan de simbolos (✓/✕/—) a
texto explicito ("Sí"/"No"/"Sin especificar"). Tras revisar v22, el
usuario pidio explicitamente que 360º vuelva a mostrar "—" cuando el
dato sea "Sin especificar" (en vez del texto completo) -- ISOFIX se
queda tal cual quedo en v22 (aunque en la practica nunca muestra "Sin
especificar": 0/30 productos).

Cambio: valor_badge() gana un parametro `sin_especificar_como_guion`
(default False). Se pasa `True` solo en la llamada de 360º
(`valor_badge(giro_360, sin_especificar_como_guion=True)`); ISOFIX
sigue llamandose sin el parametro. El resto de la logica (Sí/No como
texto plano, misma condicion de origen) no cambia.

Como consecuencia, ninguna de las dos columnas necesita ya el
ancho/padding extra que se añadio en v22 para que cupiera la palabra
"especificar" -- col-isofix/col-360 vuelven a max-width:55px sin
padding especial (su estado anterior a v22).
...
Uso:
    python tools/build_comparativa_prototipo_v23.py
"""
...
OUT = "prototipo-comparativa-sillas-v23.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v23 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v23 — <b>no es la web real</b> — misma fuente que v4-v22: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajuste sobre v22 (16-ago-2026, a petición del usuario tras revisar v22): en la columna <b>360º</b>, "Sin especificar" vuelve a mostrarse como "—" (en vez del texto completo). ISOFIX se mantiene como en v22 (texto "Sí"/"No"/"Sin especificar", aunque este último caso no se da en el dataset actual). Al no necesitar ya espacio para la palabra "especificar", ambas columnas recuperan su ancho original (76→55px). El dato y la lógica de ordenación no cambian.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v23.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v23] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}} | cabeceras ordenables: ${{sortableHeaders}} | items en menus de cabecera: ${{totalMenuItems}} | opciones select: ${{selectOptions}}`);
