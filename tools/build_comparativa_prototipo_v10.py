#!/usr/bin/env python3
"""
build_comparativa_prototipo_v10.py

Evolucion sobre v9 (ver tools/build_comparativa_prototipo_v9.py) --
MISMA fuente de datos, MISMO diseno base. Un cambio:

Arnés en la ficha tecnica pasa a ser CONDICIONAL: solo se muestra cuando
el valor tenia un matiz entre parentesis que se quito en la tabla
principal (columna "Puntos anclaje") -- es decir, cuando de verdad hay
algo mas que explicar (p.ej. "Cinturón del vehículo (3 puntos)" o el
caso "Revisar" de DUALFIX2 R, que en la tabla se acorta a la palabra
"Revisar"). Si el valor es texto llano sin parentesis (p.ej. "Arnés de
5 puntos" a secas), NO se repite en la ficha porque seria identico a lo
que ya se ve en la tabla principal -- pura duplicacion sin aportar nada
(decision tomada junto al usuario, mismo criterio de "no duplicar" que
ya se aplico con Giratoria 360º en v3).
...
Uso:
    python tools/build_comparativa_prototipo_v10.py
"""
...
OUT = "prototipo-comparativa-sillas-v10.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v10 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v10 — <b>no es la web real</b> — misma fuente que v4-v9: tools/output/auditoria_30_candidatos.json (dataset maestro consolidado 13-ago-2026, con la corrección de arnés de Nania Bogota aplicada el 14-ago-2026)</div>
<p class="proto-note">
  Cambio sobre v9: <b>Arnés en la ficha técnica pasa a ser condicional</b>. Solo aparece cuando había un matiz entre paréntesis que se quitó en la tabla principal (p.ej. "Cinturón del vehículo (3 puntos)"), o en el caso "Revisar" de DUALFIX² R. Si el valor es texto llano sin paréntesis (p.ej. "Arnés de 5 puntos" a secas), no se repite en la ficha — sería idéntico a lo que ya se ve en "Puntos anclaje", pura duplicación.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v10.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v10] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}}`);
