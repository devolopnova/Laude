#!/usr/bin/env python3
"""
build_comparativa_prototipo_v14.py

Evolucion sobre v13 (ver tools/build_comparativa_prototipo_v13.py) --
implementa UNICAMENTE el punto 6 acordado con el usuario (16-ago-2026):
un unico control "Ordenar por" (select) con 7 criterios x 2 sentidos
cada uno (14 opciones + "Sin ordenar" por defecto). Ordenacion 100%
client-side (JS), sin recargar la pagina, sin tocar filas ni datos.

Los 7 criterios y su fuente numerica:
  1. Precio            -> campo "precio" (ya numerico en el dataset)
  2. Valoracion         -> campo "valoracion" (ya numerico)
  3. Nº de valoraciones -> campo "n_val" (ya numerico)
  4. Altura             -> parseada de "altura" ("40-150 cm" -> min/max)
  5. Edad               -> parseada de "edad", normalizada a MESES para
                            poder comparar rangos con distintas unidades
                            ("15 meses - 12 años" vs "0-12 años") de
                            forma correcta; el dato mostrado en pantalla
                            no cambia, solo se calcula un valor auxiliar
                            de ordenacion.
  6. Grupo              -> parseado a "numero de etapas cubiertas"
                            (p.ej. "0+/1/2/3" -> 4) con desempate por el
                            primer grupo numerico de la secuencia.
  7. Peso               -> parseado de "peso_recomendado", usa siempre
                            el peso MAXIMO del rango.

Todos estos valores se calculan en Python (una sola vez, al generar el
HTML) y se incrustan como atributos data-sort-* en cada <tr
class="main-row">, para no reimplementar el parseo en JS. La ordenacion
en si (comparacion, direccion, regla de "Sin especificar al final",
estabilidad) vive en el <script> del HTML generado.

Regla transversal: un producto sin el dato necesario para el criterio
seleccionado se queda SIEMPRE al final (asc o desc), sin eliminarse. El
orden es estable (empate -> se conserva el orden original del dataset,
guardado en data-orig-index).

No se modifica ningun dato del dataset, ningun JSON, ni las columnas
existentes, ni la logica de "Sin especificar" en tabla/ficha. No se
toca amazon_import.py ni ningun archivo de produccion. No se
implementan filtros, busqueda, ni rediseño estetico general -- eso
queda para puntos posteriores.

Uso:
    python tools/build_comparativa_prototipo_v14.py
"""
...
OUT = "prototipo-comparativa-sillas-v14.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v14 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v14 — <b>no es la web real</b> — misma fuente que v4-v13: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo interfaz)</div>
<p class="proto-note">
  Cambio sobre v13 (punto 6, 16-ago-2026): control único <b>"Ordenar por"</b> con 7 criterios (Precio, Valoración, Nº de valoraciones, Altura, Edad, Grupo, Peso) × 2 sentidos cada uno. Ordenación 100% en el navegador, sin recargar la página; los productos sin el dato necesario para el criterio elegido se quedan siempre al final. Ninguna columna, dato ni lógica existente cambia respecto a v13.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v14.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
// / parse_peso_max en build_comparativa_prototipo_v14.py) y reordena las
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v14] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}}`);
