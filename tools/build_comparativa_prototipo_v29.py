#!/usr/bin/env python3
"""
build_comparativa_prototipo_v29.py

Evolucion sobre v28 (ver tools/build_comparativa_prototipo_v28.py) --
implementa UNICAMENTE el punto 7.11 acordado con el usuario
(16-ago-2026), con dos cambios exclusivos de "Rango de uso":

  1. Centrado vertical del bloque completo: las 2-4 lineas (Edad/
     Altura/Grupo/Peso) se envuelven ahora en un <div class="rango-
     block">, para que el centrado (via vertical-align:middle,
     heredado de "tbody td") actue sobre el bloque COMO UNIDAD -- no
     linea a linea -- sea cual sea el numero de lineas presentes (2,
     3 o 4). No se toca la jerarquia tipografica (Edad/Altura en
     negrita, Grupo/Peso normal) ni el espaciado entre lineas.
  2. Normalizacion visual de "Recién nacido" -> "0" en la linea de
     Edad (nueva funcion normalizar_recien_nacido()) -- solo cambia lo
     que se pinta en pantalla; el dato del dataset y el calculo de
     ordenacion por Edad (que ya trataba "recién nacido" como 0 meses,
     de forma independiente) no se tocan.
...
Arnés), la ordenacion, el selector "Ordenar por", la ficha tecnica,
los botones Amazon, ni los enlaces/ASIN. No se toca amazon_import.py
ni ningun archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v29.py
"""
...
OUT = "prototipo-comparativa-sillas-v29.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v29 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v29 — <b>no es la web real</b> — misma fuente que v4-v28: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo Rango de uso)</div>
<p class="proto-note">
  Ajuste sobre v28 (punto 7.11, 16-ago-2026): en <b>Rango de uso</b>, el bloque completo (Edad/Altura/Grupo/Peso) queda centrado verticalmente como una sola unidad dentro de la fila, y "Recién nacido" se muestra como "0" en la línea de Edad. El resto de la columna, la tipografía, el espaciado y cualquier otro dato no se han tocado.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v29.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
