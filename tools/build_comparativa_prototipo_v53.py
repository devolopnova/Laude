#!/usr/bin/env python3
"""
build_comparativa_prototipo_v53.py

Evolucion sobre v52 (ver tools/build_comparativa_prototipo_v52.py) --
implementa UNICAMENTE el rediseño de las tarjetas de resultado del
panel movil "Selecciona una sillita" acordado con el usuario
(18-ago-2026). Cambia SOLO build_mobile_result_card_html() (+ su CSS
asociada .mc-result-*) -- no toca la tarjeta VS (.mc-product-*), el
chip "Sillita elegida" (.mc-summary-*) ni ninguna otra pieza del
selector.

Tres cambios:

  1. Imagen mucho mas protagonista: .mc-result-thumb pasa de 56px a
     96px (salto claro, no un ligero aumento), radio 9px->12px. El
     fallback de iniciales (.mc-thumb-ph) usa la MISMA clase, asi que
     reserva automaticamente el mismo espacio exista o no imagen real
     -- todas las tarjetas mantienen la misma estructura.
     object-fit:contain (ya estaba) sigue evitando deformar/recortar.

  2. Informacion reorganizada por etiquetas en vez de una linea unica
     con puntos separadores ("0-12 años · 40-150 cm · Grupo.../Hasta
     36kg"): ahora "Rango de uso" / "Altura" / "Grupo / peso", cada uno
     en su propia linea, con la MISMA regla que ya rige Edad/Altura/
     Peso/Grupo en la matriz principal -- reutiliza literalmente
     _mc_rango_campo() (sin logica de formato nueva): sin dato, esa
     linea no se genera (nunca se inventa ni se deja una etiqueta
     huerfana). Funcion nueva _mc_result_field(label, *valores) monta
     el bloque etiqueta+valor(es), usada tambien para agrupar Grupo y
     Peso bajo la misma etiqueta "Grupo / peso" (una linea por cada
     uno que exista).

  3. Se elimina ISOFIX de esta tarjeta (ya esta en la matriz de
     comparacion) -- el precio queda solo, sin la etiqueta
     "ISOFIX: Sí/No" al lado. El dato ISOFIX en si, `valor_badge()` y
     su uso en la tabla de PC/la matriz/la ficha tecnica NO se tocan.

Orden final del contenido (jerarquia pedida): foto -> marca+modelo ->
rango de uso -> altura -> grupo/peso -> valoracion+nº valoraciones ->
precio.

No se toca: el buscador, la logica de asignacion de slot (Slot1/Slot2,
sustitucion del activo, exclusion del mismo ASIN), el boton ✕, el
dataset, la logica de comparacion, la matriz de caracteristicas, la
ficha tecnica, la tabla de PC, las imagenes ya existentes ni las
ordenaciones.

Uso:
    python tools/build_comparativa_prototipo_v53.py
"""
...
OUT = "prototipo-comparativa-sillas-v53.html"
...
<title>PROTOTIPO v53 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v53 — <b>no es la web real</b> — misma fuente que v4-v52: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Tarjetas de resultado rediseñadas (18-ago-2026): en el panel "Selecciona una sillita", imagen protagonista (56px→96px, mismo espacio con o sin foto real), datos por etiquetas ("Rango de uso"/"Altura"/"Grupo / peso" en líneas separadas, sin líneas huérfanas cuando falta el dato) y sin ISOFIX (ya está en la matriz). Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v53.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
