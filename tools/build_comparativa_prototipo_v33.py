#!/usr/bin/env python3
"""
build_comparativa_prototipo_v33.py

Evolucion sobre v32 (ver tools/build_comparativa_prototipo_v32.py) --
implementa UNICAMENTE el Ajuste acordado con el usuario (18-ago-2026):
la interaccion de la cabecera "Valoración" cambia de un toggle de 2/3
estados a un MENU con dos criterios INDEPENDIENTES, replicando el mismo
patron ya usado por "Orientación"/"Arnés" (menu compacto de botones,
build_simple_menu()):

  - Valoración: mayor → menor / menor → mayor.
  - Nº valoraciones: mayor → menor / menor → mayor.

Ninguno de los dos criterios desempata con el otro -- en caso de empate
(misma puntuacion al ordenar por Valoración, o mismo nº de valoraciones
al ordenar por Nº de valoraciones) se conserva el orden original del
dataset (estable). Esto revierte el desempate cruzado introducido en
v31/v32 (Ajustes 7.16/7.17): esa logica dedicada desaparece por
completo, ya que ahora "valoracion-desc"/"valoracion-asc" son un
criterio simple mas dentro de SIMPLE_CRITERIA (igual que Precio/Peso),
sin rama especial. Las claves "nval-desc"/"nval-asc" ya existian en el
motor de ordenacion desde antes (v30 llego a quitarles su unico punto
de entrada en la interfaz al eliminar el <select> "Ordenar por") -- este
Ajuste les da un punto de entrada nuevo, dentro del propio menu de
Valoración, sin crear columna ni selector nuevos.

La cabecera sigue diciendo "VALORACIÓN" (sin cambios de texto) y el
icono ↕ se resalta igual que en Rango de uso/Orientación/Arnés cuando
cualquiera de los 4 items del menu esta activo (keyGroup() agrupa
valoracion-asc/desc y nval-asc/desc bajo el mismo grupo "valoracion").
...
Uso:
    python tools/build_comparativa_prototipo_v33.py
"""
...
OUT = "prototipo-comparativa-sillas-v33.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v33 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v33 — <b>no es la web real</b> — misma fuente que v4-v32: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajuste (18-ago-2026): <b>Valoración</b> pasa a un menú con dos criterios independientes — Valoración y Nº de valoraciones —, cada uno con sus 2 sentidos, sin desempate cruzado entre ellos (en empate se conserva el orden original). La cabecera sigue diciendo "Valoración", sin columna nueva.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v33.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
