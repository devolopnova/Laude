#!/usr/bin/env python3
"""
build_comparativa_prototipo_v61.py

Evolucion sobre v60 (ver tools/build_comparativa_prototipo_v60.py) --
amplia UNICAMENTE el recorte de "Puntos anclaje" (arnés) de la tabla PC
introducido en v60 para que tambien cubra el separador "/", no solo "+"
(26-ago-2026, mismo dia): v60 solo cubria el caso Chicco Everone i-Size
("Arnés de 5 puntos (...) + cinturón de 3 puntos (...)"), pero el
usuario señalo que Lionelo LEVI ONE i-Size tiene el MISMO problema con
otro separador: "Arnés de 5 puntos (76-105cm) / cinturón de 3 puntos
(100-150cm)" -- la tabla seguia mostrando "Arnés 5 puntos / cinturón 3
puntos" sin recortar, porque la regex de v60 solo buscaba "+".

Cambio (unico, Python, dentro de `arnes_compacto_tabla_pc()` -- la
misma funcion exclusiva de PC creada en v60, nada mas se toca):
  - La regex de corte pasa de `r'\\s*\\+.*$'` (solo "+") a
    `r'\\s*[/+]\\s*cinturón.*$'` (case-insensitive): corta desde un "+"
    O UN "/" seguido de la palabra "cinturón" en adelante. Anclar el
    corte a la palabra "cinturón" (no a cualquier "+"/"/" suelto) evita
    recortar de mas si algun valor futuro usara esos caracteres por otro
    motivo no relacionado con un segundo sistema de sujecion.
  - Afecta a los 2 productos del dataset que combinan arnés+cinturón:
    Chicco Everone i-Size (con "+", ya corregido en v60, sigue igual) y
    Lionelo LEVI ONE i-Size (con "/", nuevo con este cambio). El resto
    de productos no tienen ninguno de los dos patrones, así que sus
    celdas no cambian.
...
`arnes_compacto_tabla()` sin modificar y no se toca -- igual que en v60.

Uso:
    python tools/build_comparativa_prototipo_v61.py
"""
...
OUT = "prototipo-comparativa-sillas-v61.html"
...
def arnes_compacto_tabla_pc(valor):
    ...
    sujeción unido con "+" o "/" (p.ej. 'Arnés 5 puntos + cinturón 3
    puntos vehículo' -> 'Arnés 5 puntos'; 'Arnés 5 puntos / cinturón 3
    puntos' -> 'Arnés 5 puntos'), a petición explícita del usuario
    (26-ago-2026, ampliado el mismo día tras ver el caso Lionelo LEVI
    ONE i-Size que usa "/" en vez de "+"): en la tabla principal solo
    debe verse el sistema de arnés en sí, nunca el matiz del cinturón --
    ese matiz se queda exclusivamente en la ficha técnica, que ya lo
    muestra completo via arnes_tiene_matiz_oculto() (sin cambios, sigue
    detectando la diferencia contra el valor íntegro). El corte se
    ancla a la palabra "cinturón" (no a cualquier "+"/"/" suelto) para
    no recortar de más si algún futuro valor usa esos caracteres por
    otro motivo. NO se usa en móvil -- su matriz de comparación sigue
    llamando a arnes_compacto_tabla() tal cual, sin este recorte
    adicional; este ajuste es exclusivo de PC."""
    texto, matiz = arnes_compacto_tabla(valor)
    if texto in (GUION, "Revisar"):
        return texto, matiz
    recortado = re.sub(r'\s*[/+]\s*cinturón.*$', '', texto, flags=re.IGNORECASE).strip()
...
<title>PROTOTIPO v61 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v61 — <b>no es la web real</b> — misma fuente que v4-v60: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ampliación (26-ago-2026): el recorte de "Puntos anclaje" de v60 (tabla PC) ahora también cubre el separador "/", no solo "+" — Lionelo LEVI ONE i-Size ("Arnés 5 puntos / cinturón 3 puntos") también queda solo en "Arnés 5 puntos", igual que Chicco Everone. El detalle completo sigue en la ficha técnica. La matriz de comparación móvil no cambia.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v61.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
