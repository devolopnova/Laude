#!/usr/bin/env python3
"""
build_comparativa_prototipo_v25.py

Evolucion sobre v24 (ver tools/build_comparativa_prototipo_v24.py) --
implementa UNICAMENTE el punto 7.8.x acordado con el usuario
(16-ago-2026): ajuste visual INTERNO de la ficha tecnica (v24), sin
tocar nada fuera de ella.

Problema corregido: en v24, `.detail-item` usaba `justify-content:
space-between`, lo que empujaba el valor de cada campo al extremo
derecho de su columna (1fr de ancho), dejando un hueco enorme entre el
nombre ("Reclinable") y su valor ("Sin especificar"). Nombre y valor
se percibian como elementos independientes en vez de una unidad.

Cambio (solo CSS, ningun HTML/estructura/logica nueva):
  - `.detail-item`: `justify-content:space-between` -> `flex-start`
    con `gap:10px` fijo -- el valor queda pegado al nombre con una
    separacion corta y CONSTANTE en todas las filas, en vez de
    disparado al borde derecho de la columna.
  - `.detail-value`: deja de estar en negrita (font-weight 600 -> 400,
    "peso normal" pedido explicitamente) y de alinearse a la derecha
    (text-align right -> left, ya no tiene sentido sin el
    space-between). `.detail-label` no cambia (semibold, ya cumplia lo
    pedido).
  - `.detail-value--unk` (Sin especificar) no cambia: gris + cursiva,
    ya era el tratamiento correcto.

No se toca el HTML generado (misma estructura de detail_item/
FICHA_CAMPOS_PRINCIPALES/particularidades que v24), ningun dato del
dataset, ningun JSON, la tabla principal, ninguna columna, la cabecera
del punto 7.1, la ordenacion, el selector "Ordenar por", ni el boton
"Ver en Amazon". No se toca amazon_import.py ni ningun archivo de
produccion.

Uso:
    python tools/build_comparativa_prototipo_v25.py
"""
...
OUT = "prototipo-comparativa-sillas-v25.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v25 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v25 — <b>no es la web real</b> — misma fuente que v4-v24: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo CSS interno de la ficha técnica)</div>
<p class="proto-note">
  Ajuste sobre v24 (punto 7.8.x, 16-ago-2026): dentro de la <b>ficha técnica</b>, el nombre de cada característica y su valor quedan juntos como una unidad — antes "justify-content: space-between" empujaba el valor al extremo derecho de la columna, dejando un hueco enorme. Ahora hay una separación fija y corta (10px) entre nombre y valor en todas las filas. "Sin especificar" se mantiene como texto (gris, cursiva). No se ha tocado ningún dato, campo, la tabla principal, la cabecera ni la ordenación.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v25.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
