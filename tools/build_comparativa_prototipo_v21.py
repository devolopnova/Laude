#!/usr/bin/env python3
"""
build_comparativa_prototipo_v21.py

Evolucion sobre v20 (ver tools/build_comparativa_prototipo_v20.py) --
UNIFICACION TIPOGRAFICA (16-ago-2026) de todas las columnas de
informacion: Normativa, Rango de uso, ISOFIX, Orientación, 360º, Arnés
y Valoración pasan a compartir el mismo TAMAÑO BASE (12.5px), tomando
como referencia el tamaño que ya tenia Normativa ("R129 / i-Size"). La
jerarquia visual se consigue SOLO con font-weight (negrita donde tiene
sentido), nunca con tamaños de fuente distintos -- se elimina el
"ruido visual" de tener 4-5 tamaños distintos en la misma fila.

Cambios de tamaño (todo lo demas -- estructura, columnas, ordenacion,
cabecera, ficha tecnica, enlaces -- sin tocar):
  - Rango de uso: Edad/Altura 14px -> 12.5px (sigue en negrita, 700);
    Grupo/Peso 12px -> 12.5px y peso 500 -> 400 (normal, sin negrita).
    Los 4 comparten ahora el mismo tamaño base; la jerarquia es solo
    negrita vs. normal.
  - Valoración: la puntuacion (⭐ 4,5) baja de 16px a 12.5px, sigue en
    negrita para distinguirse, ya no domina la fila. El nº de
    valoraciones NO cambia (sigue en 11.5px, excepcion explicita del
    usuario por ser dato secundario).
  - Orientación: 13px -> 12.5px.
  - ISOFIX/360º (glifo ✓/✕): 16px -> 14px -- se reduce porque dominaba
    demasiado, pero se deja un pelin por encima del tamaño base (12.5px)
    por ser un simbolo, no texto, para que se siga identificando bien;
    se mantiene la diferencia de color entre Sí/No/Sin especificar.
  - Normativa y Arnés ya estaban en 12.5px -- sin cambios (son la
    referencia).

El espaciado entre lineas de Rango de uso (selectores de hermano
adyacente, 2px/5px) NO cambia respecto a v20.

No se modifica ningun dato del dataset, ningun JSON, ninguna columna,
anchura, orden, ficha tecnica, boton "Ver en Amazon", la ordenacion, ni
la cabecera del punto 7.1. No se toca amazon_import.py ni ningun
archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v21.py
"""
...
OUT = "prototipo-comparativa-sillas-v21.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v21 — Comparativa sillitas de coche</title>
...
/* ISOFIX/360º: el glifo ✓/✕ baja de 16px a 14px -- sigue un poco por
   encima del tamaño base (12.5px) porque es un simbolo, no texto, pero
   ya no domina la fila. Se mantiene la diferencia de color entre
   Sí/No/Sin especificar. */
td.col-isofix,td.col-360{{text-align:center;max-width:55px;}}
.chk{{display:inline-block;font-size:14px;font-weight:700;line-height:1;}}
...
<div class="proto-banner">⚠ Prototipo v21 — <b>no es la web real</b> — misma fuente que v4-v20: tools/output/auditoria_30_candidatos.json (mismo dataset que v19/v20, sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Cambio sobre v20 (16-ago-2026): unificación tipográfica de las columnas de información — Normativa, Rango de uso, ISOFIX, Orientación, 360º, Arnés y la puntuación de Valoración comparten ahora el mismo <b>tamaño base 12.5px</b> (el que ya tenía Normativa). La jerarquía se consigue solo con negrita (Edad/Altura en Rango de uso, la puntuación ⭐) frente a peso normal — ya no hay 4-5 tamaños distintos en la misma fila. El nº de valoraciones sigue más pequeño (excepción explícita). Ningún dato, columna, ficha técnica, enlace ni la ordenación se han tocado.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v21.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v21] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}} | cabeceras ordenables: ${{sortableHeaders}} | items en menus de cabecera: ${{totalMenuItems}} | opciones select: ${{selectOptions}}`);
