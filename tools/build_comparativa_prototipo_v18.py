#!/usr/bin/env python3
"""
build_comparativa_prototipo_v18.py

Evolucion sobre v17 (ver tools/build_comparativa_prototipo_v17.py) --
implementa UNICAMENTE el punto 7.2 acordado con el usuario (16-ago-2026):
mejora estetica de la celda "Valoración" en la tabla principal.

Cambios (solo presentacion, ningun dato ni logica):
  1. Jerarquia visual clara: la puntuacion (⭐ 4,8) pasa de 13.5px a
     16px y sigue en negrita -- dato protagonista. El nº de
     valoraciones pasa de 11px a 11.5px, mantiene peso normal y color
     mas suave (--ink-soft) -- dato secundario, pero legible.
  2. El nº de valoraciones ahora se acompaña de la palabra
     "valoraciones" en la celda de la tabla (antes solo el numero
     suelto; la ficha tecnica ya lo hacia asi) -- mismo valor
     (fmt_n_val), solo se le añade la etiqueta.
  3. Los dos datos quedan centrados horizontalmente en la columna
     (antes alineados a la izquierda), con mas separacion vertical
     entre ellos (margin-top 2px -> 4px) sin ensanchar la fila.
  4. La columna se ensancha ligeramente (70 -> 84px, +14px) -- lo
     minimo necesario para que "valoraciones" no desborde ni se corte
     a media palabra. Es el UNICO ancho que cambia.

No se toca: el valor numerico interno usado para ordenar (sigue siendo
p.get("valoracion")/p.get("n_val") sin formatear, exactamente igual que
en v14-v17), la cabecera del punto 7.1 (Valoración sigue con su ↕
clicable, sin cambios), la ficha tecnica, los botones "Ver en Amazon",
ninguna otra columna, ni el dataset/JSON. No se toca amazon_import.py
ni ningun archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v18.py
"""
...
OUT = "prototipo-comparativa-sillas-v18.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v18 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v18 — <b>no es la web real</b> — misma fuente que v4-v17: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo estética de la celda Valoración)</div>
<p class="proto-note">
  Cambio sobre v17 (punto 7.2, 16-ago-2026): jerarquía visual en la celda <b>Valoración</b> — la puntuación (⭐ 4,8) crece y queda como dato protagonista, el nº de valoraciones (con la palabra "valoraciones" añadida) queda claramente secundario debajo, ambos centrados. Columna ligeramente más ancha (70→84px) para que quepa el texto. Los valores numéricos, la ordenación por Valoración/Nº de valoraciones y la cabecera del punto 7.1 no se han tocado.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v18.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v18] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}} | cabeceras ordenables: ${{sortableHeaders}} | items en menus de cabecera: ${{totalMenuItems}} | opciones select: ${{selectOptions}}`);
