...
BRANDS: "OrderedDict[str, List[str]]" = OrderedDict([
    ...
    ("Klippan", ["klippan"]),
    ("Babify", ["babify"]),
    # Añadidas el 31-ago-2026: estas 5 marcas YA tenian productos en el
    # catalogo (tools/output/auditoria_30_candidatos.json) pero no estaban
    # aqui, asi que filter_by_brand() descartaba todos sus anuncios y una
    # busqueda por esa marca devolvia 0 resultados (detectado buscando
    # KikkaBoo, que devolvio 40 productos de la competencia y ninguno suyo).
    ("KikkaBoo", ["kikkaboo", "kikka boo"]),
    ("Lionelo", ["lionelo"]),
    ("Graco", ["graco"]),
    ("Nania", ["nania"]),
    ("Jovikids", ["jovikids", "jovi kids"]),
])
...
def campo(original=None, normalizado=None, mostrar=None, fuente=None, url_fuente=None,
          revisar=False, texto_fuente=None, confianza=None,
          contradiccion_detectada=False, valor_amazon=None, valor_fabricante=None,
          motivo_contradiccion=None) -> Dict:
    ...
    Trazabilidad de contradiccion (añadida 2026-08-11, regla de "prioridad
    del fabricante"): `contradiccion_detectada` es True cuando Amazon y
    fabricante afirmaron explicitamente valores distintos para el MISMO
    atributo del MISMO producto (se haya podido resolver a favor del
    fabricante o mediante decision manual, o no). `valor_amazon` /
    `valor_fabricante` conservan ambas evidencias aunque el valor final
    (`normalizado`/`mostrar`) proceda de una sola. `motivo_contradiccion` es
    el texto explicativo de por que se considero contradiccion y como se
    resolvio. Estos 4 campos son principalmente para uso interno/auditoria
    -- no cambian el significado de `revisar` (que sigue indicando "sin
    resolver, pendiente de revision humana"; una contradiccion puede quedar
    con revisar=False si ya se resolvio via prioridad del fabricante o via
    decision manual). Por ahora solo los rellena resolve_grupo_r44 con
    valores reales; el resto de atributos los reciben con su valor neutro
    por defecto (False/None) sin cambiar su comportamiento."""
    return {
        "original": original,
        ...
        "valor_amazon": valor_amazon,
        "valor_fabricante": valor_fabricante,
        "motivo_contradiccion": motivo_contradiccion,
    }
...
GRUPO_RE = re.compile(r"\b(?:grupos?|groups?)\s+([0-3]\+?(?:[/\-\s]?[0-3]\+?){0,3})(?=[^0-9]|$)")


def _amazon_grupo(raw: Dict) -> Tuple[Optional[str], Optional[str]]:
    """Grupo (0/0+/1/2/3, clasificacion DISTINTA de la homologacion R129/
    R44 -- ver resolve_grupo_r44) solo si aparece EXPLICITO como 'Grupo X'
    o 'Group X' en el texto. Nunca se infiere de edad/peso/altura/
    normativa/tipo de silla. Recorre TODOS los origenes (tabla > bullets >
    titulo > descripcion) y se queda con la primera mencion explicita
    encontrada.

    El patron acepta separador '/', '-', espacio o ninguno entre digitos
    consecutivos (cubre 'Grupo 1/2/3', 'GRUPO 0 1 2 3' y 'GRUPO 0+123' tal
    cual aparecen en Amazon), y siempre preserva el '+' solo donde el texto
    origen lo trae -- nunca se añade un '+' que no esta en la fuente (caso
    auditado: Bebeconfort RevolveFix Plus 360, 'GRUPO 0 1 2 3' sin '+' ->
    se muestra '0/1/2/3', no '0+/1/2/3')."""
    for _origin_label, text in build_origins(raw):
        t = _norm(text)
        m = GRUPO_RE.search(t)
        if not m:
            continue
        segmentos = re.findall(r"[0-3]\+?", m.group(1))
        if not segmentos:
            continue
        return "/".join(segmentos), text
    return None, None


def _mostrar_grupo(v: Optional[str]) -> str:
    return f"Grupo {v}" if v else "Sin especificar"


def resolve_grupo_r44(raw: Dict, fab_entry: Optional[Dict], asin: str) -> Dict:
    """Grupo (clasificacion 0/0+/1/2/3), auditado el 2026-08-11 y revisado
    el mismo dia con la regla de "prioridad del fabricante". Pese al nombre
    historico del campo (grupo_r44), este atributo es INDEPENDIENTE de la
    homologacion: representa la etiqueta comercial de "Grupo X" que muchas
    fichas -- incluidas las de sillas R129/i-Size -- siguen usando de forma
    informal para describir las etapas de uso del producto. No confundir
    con `normativa` (R129/i-Size vs R44), que se resuelve por separado en
    resolve_normativa y no cambia con esta funcion. Nunca se infiere un
    grupo a partir de edad/peso/altura/normativa/tipo de silla -- solo
    evidencia textual explicita de "Grupo X"/"Group X" (Amazon y/o
    fabricante).

    Regla de prioridad del fabricante (contradiccion real, Amazon y
    fabricante afirman EXPLICITAMENTE grupos distintos para el mismo
    producto/variante): se usa el valor del fabricante como definitivo
    (confianza alta), pero se registra la contradiccion en
    `contradiccion_detectada`/`valor_amazon`/`valor_fabricante`/
    `motivo_contradiccion` -- nunca se oculta que hubo desacuerdo aunque el
    valor final proceda de una sola fuente. Si el fabricante simplemente NO
    menciona el grupo (silencio, no contradiccion), se conserva el dato
    explicito de Amazon sin marcar contradiccion.

    Estados posibles (nunca "Si"/"No"/"No determinado" para este atributo,
    no es booleano):
      - Valor concreto ('0', '0+', '1', '2/3', '1/2/3', '0+/1/2/3', etc.):
        evidencia explicita y clara (Amazon, fabricante, o ambas si
        coinciden; o el valor del fabricante si hubo contradiccion real).
      - "Sin especificar": ninguna fuente disponible menciona un grupo
        explicito tras la auditoria (o, caso especial documentado abajo,
        se descarta editorialmente una mencion de Amazon no fiable).

    Caso especial (decision editorial explicita del usuario, 2026-08-11):
    Bebeconfort Marvel RoadSafe (B0GGZYY3BZ) tiene varias menciones de
    "grupo 2-3" en Amazon, pero una de ellas esta en el mismo bullet que ya
    demostro contener una afirmacion de ISOFIX falsa (el fabricante
    confirma que el modelo NO lleva ISOFIX, ver hallazgo "isofix" de este
    mismo ASIN en el lookup). El fabricante tampoco menciona grupo para
    este modelo (silencio, no contradiccion directa sobre "grupo" en si).
    Por decision explicita del usuario, no se usa la mencion de Amazon como
    dato definitivo -- se registra en el lookup como hallazgo "grupo" con
    resuelto_manualmente=True y valor=None, y esta funcion lo traduce a
    "Sin especificar" con la contradiccion marcada internamente (ver rama
    resuelto_manualmente abajo). No es una resolucion automatica del
    sistema."""
    amazon_valor, amazon_texto = _amazon_grupo(raw)
    fab_hallazgo, fab_url, identidad_no_confirmada = _fab_hallazgo(fab_entry, "grupo")
    ...
    # Decision manual ya documentada en el lookup (p.ej. Marvel RoadSafe):
    # se respeta tal cual, nunca se recalcula automaticamente. Si el valor
    # manual es None (como Marvel), el resultado es "Sin especificar" pero
    # con la contradiccion que motivo la decision registrada igualmente.
    if fab_hallazgo and fab_hallazgo.get("resuelto_manualmente"):
        fuente = "fabricante_oficial (confirmado manualmente)" if fab_valor else \
            "amazon_y_fabricante (resuelto manualmente)"
        return campo(amazon_texto or fab_texto, fab_valor, _mostrar_grupo(fab_valor),
                     fuente=fuente, url_fuente=fab_url, texto_fuente=fab_texto, confianza="alta",
                     contradiccion_detectada=True, valor_amazon=amazon_valor, valor_fabricante=fab_valor,
                     motivo_contradiccion=fab_texto)

    if identidad_no_confirmada and amazon_valor is None:
        return campo(None, None, "Posible variante/producto diferente — no confirmado",
                     fuente=None, url_fuente=fab_url, revisar=True)

    # Contradiccion real (Caso A/C): Amazon y fabricante afirman grupos
    # EXPLICITOS distintos -> prioridad del fabricante, pero la
    # contradiccion queda registrada, no oculta.
    if amazon_valor is not None and fab_valor is not None and fab_valor != amazon_valor:
        motivo = (
            f"Contradiccion real detectada: Amazon afirma 'Grupo {amazon_valor}' "
            f"({amazon_texto!r}) mientras que la ficha oficial del fabricante afirma "
            f"'Grupo {fab_valor}' ({fab_texto!r}). Resuelto a favor del fabricante "
            f"(regla de prioridad del fabricante, 2026-08-11): la web oficial es la fuente "
            f"mas fiable para especificaciones tecnicas verificables cuando hay desacuerdo "
            f"explicito con Amazon."
        )
        return campo(fab_texto, fab_valor, _mostrar_grupo(fab_valor), fuente="fabricante_oficial",
                     url_fuente=fab_url, texto_fuente=fab_texto, confianza="alta",
                     contradiccion_detectada=True, valor_amazon=amazon_valor, valor_fabricante=fab_valor,
                     motivo_contradiccion=motivo)

    # Caso D (coinciden) o Caso B (solo Amazon, fabricante en silencio):
    # ninguno de los dos es una contradiccion.
    if amazon_valor is not None:
        coincide = fab_valor is not None and fab_valor == amazon_valor
        fuente = "amazon_y_fabricante" if coincide else "amazon"
        return campo(amazon_texto, amazon_valor, _mostrar_grupo(amazon_valor),
                     fuente=fuente, url_fuente=fab_url if coincide else None,
                     texto_fuente=amazon_texto, confianza="alta" if coincide else "media",
                     valor_amazon=amazon_valor, valor_fabricante=fab_valor)

    # Caso E (solo el fabricante lo confirma, Amazon en silencio): dato de
    # primer nivel, no "inferido".
    if fab_valor is not None:
        return campo(fab_texto, fab_valor, _mostrar_grupo(fab_valor), fuente="fabricante_oficial",
                     url_fuente=fab_url, texto_fuente=fab_texto, confianza="alta",
                     valor_amazon=amazon_valor, valor_fabricante=fab_valor)
    ...
    if _fab_identidad_valida(fab_entry):
        texto = ("Auditado en Amazon y en la ficha oficial del fabricante (identidad "
                 "verificada): ninguna mencion explicita de 'Grupo X'/'Group X' en ninguna "
                 "fuente. La ausencia de mencion no implica ningun grupo concreto, y nunca se "
                 "infiere de edad/peso/altura/normativa.")
        return campo(None, None, "Sin especificar", fuente="amazon", confianza="alta", texto_fuente=texto)

    texto = ("Auditado en Amazon (revision exhaustiva de titulo, bullets, descripcion y tabla): "
             "ninguna mencion explicita de 'Grupo X'/'Group X'. Sin ficha oficial del fabricante "
             "disponible como segunda fuente (identidad no verificada para este ASIN). La "
             "ausencia de mencion no implica ningun grupo concreto.")
    return campo(None, None, "Sin especificar", fuente="amazon", confianza="media", texto_fuente=texto)
...
def build_clasificacion(raw: Dict, asin: str, fabricante_lookup: Dict) -> Dict:
    ...
    return {
        ...
        "grupo_r44": resolve_grupo_r44(raw, fab_entry, asin),
