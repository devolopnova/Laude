#!/usr/bin/env python3
"""
build_comparativa_prototipo_v57.py

Evolucion sobre v56 (ver tools/build_comparativa_prototipo_v56.py) --
implementa UNICAMENTE la conexion de las fotografias de producto ya
descargadas en images/sillas-coche/ con las tarjetas del selector movil
(corregido a peticion del usuario, 26-ago-2026).

Problema: `build_mobile_thumb_html()` (la unica funcion que decide si
una tarjeta movil muestra foto real o el fallback de iniciales -- la
reutilizan la tarjeta VS, el chip "Sillita elegida" y la tarjeta de
resultado del buscador) miraba un diccionario `DEMO_COMPARE_IMAGES`
hardcodeado con solo 2 ASIN (los 2 productos de ejemplo iniciales, de
cuando la comparativa VS aun no tenia selector). Los otros 23 productos
que ya se descargaron y validaron despues (ver auditoria de imagenes,
26-ago-2026) seguian sin aparecer nunca porque el diccionario nunca se
actualizo -- no era un problema de las imagenes en si (esas ya estaban
bien en disco), sino de que el prototipo no las buscaba.

Cambio (unico, Python, SOLO en la resolucion de imagen de las tarjetas
moviles -- ni CSS ni el diseno de la tarjeta se tocan):
  - `DEMO_COMPARE_IMAGES` (dict hardcodeado de 2 ASIN) se sustituye por
    `MOBILE_PRODUCT_IMAGES`, calculado dinamicamente al generar la
    pagina: para cada uno de los 30 ASIN del dataset, comprueba si
    existe `images/sillas-coche/<ASIN>.webp` en disco (`os.path.exists`)
    y, si existe, lo usa; si no, sigue exactamente el mismo fallback de
    iniciales de siempre. Asi el prototipo refleja SIEMPRE el estado
    real de la carpeta de imagenes, sin mantener a mano una lista que se
    desincroniza cada vez que se descarga una imagen nueva.
  - `build_mobile_thumb_html()` pasa a consultar `MOBILE_PRODUCT_IMAGES`
    en vez de `DEMO_COMPARE_IMAGES` -- misma logica exacta (imagen si
    existe, si no iniciales), mismo `<img>` con las mismas clases/
    proporciones/`object-fit:contain` que ya tenia para los 2 ASIN de
    ejemplo, ahora aplicado a los 25 productos que ya tienen foto.
  - No se descarga, sustituye ni modifica ninguna fotografia: las 25
    que ya estaban en images/sillas-coche/ se usan tal cual.
  - `DEMO_COMPARE_ASINS` (que decide que 2 productos se renderizan como
    ejemplo inicial de la tarjeta VS en el servidor) no se toca -- es un
    dato distinto, no relacionado con la resolucion de imagen.

No se toca la matriz principal, la ficha tecnica, el buscador, el
filtro de edad, el selector Slot 1/Slot 2, la tabla de PC, las
ordenaciones, los datos ni los enlaces de Amazon.
...
OUT = "prototipo-comparativa-sillas-v57.html"
...
# Imagenes de producto (corregido 26-ago-2026, v57): las fotos reales de
...
# el fallback de iniciales de build_mobile_thumb_html(), sin necesidad de
# listar aqui los "sin foto".
MOBILE_IMAGES_DIR = "images/sillas-coche"
MOBILE_PRODUCT_IMAGES = {
    os.path.splitext(f)[0]: f"{MOBILE_IMAGES_DIR}/{f}"
    for f in (os.listdir(MOBILE_IMAGES_DIR) if os.path.isdir(MOBILE_IMAGES_DIR) else [])
...
def build_mobile_thumb_html(p, img_cls, ph_cls=""):
    ...
    (MOBILE_PRODUCT_IMAGES, ver comentario mas arriba -- 25 de los 30
    productos la tienen a 26-ago-2026), o si no el MISMO fallback que ya
    usa la tabla de PC para TODOS sus productos: un recuadro con las
    iniciales de la marca (iniciales()), nunca un placeholder
    generico/inventado."""
    marca = p.get("marca") or SIN_ESPECIFICAR
    modelo = p.get("modelo") or SIN_ESPECIFICAR
    alt = html_lib.escape(f"{marca} {modelo}")
    imagen = MOBILE_PRODUCT_IMAGES.get(p["asin"])
    if imagen:
...
<title>PROTOTIPO v57 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v57 — <b>no es la web real</b> — misma fuente que v4-v56: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Corrección (26-ago-2026): las tarjetas del selector móvil (VS, "Sillita elegida" y resultados del buscador) ahora muestran la fotografía real de <b>images/sillas-coche/&lt;ASIN&gt;.webp</b> cuando existe (25 de los 30 productos) en vez del fallback de iniciales — antes solo se comprobaban 2 ASIN de ejemplo. Los 5 productos sin foto validada siguen mostrando iniciales. Ni las imágenes ni el resto del diseño/datos cambian. Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v57.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
