#!/usr/bin/env python3
"""
build_comparativa_prototipo_v59.py

Evolucion sobre v58 (ver tools/build_comparativa_prototipo_v58.py) --
implementa UNICAMENTE un efecto hover para ampliar la foto de producto
en la tabla de escritorio (PC), a peticion del usuario (26-ago-2026):
las miniaturas de 44x44px se ven pequenas, pero agrandarlas de forma
permanente haria crecer las filas y empeoraria la densidad de la tabla
-- la solucion es un popup flotante que aparece solo al pasar el raton
(o dar foco con teclado) por encima, sin tocar el tamano de la fila.

Cambio (unico, CSS + JS -- nada de Python/HTML de `build_row()` ni del
resto del prototipo se toca; la miniatura 44x44px de v58 sigue exacta):
  - CSS nuevo: `.prod-thumb-img{{cursor:zoom-in;}}` (una propiedad
    añadida sin tocar la regla de v58) + `.prod-zoom-popup` (el propio
    popup: 200x200px, fondo blanco, borde sutil, esquinas redondeadas,
    sombra suave, `object-fit:contain` -- como todas las fotos ya son
    cuadradas de origen (pad_to_square, 600x600), un popup cuadrado fijo
    nunca deforma ni recorta). `position:fixed` + `pointer-events:none`
    + transicion de opacidad/escala de 0.15s.
  - JS nuevo: una IIFE autocontenida que crea el popup una sola vez
    (`document.createElement`, `appendChild` a `<body>`) y añade
    listeners `mouseenter`/`mouseleave`/`focus`/`blur` a cada
    `.prod-thumb-img` (SOLO las fotos reales -- `.prod-thumb-ph`, el
    fallback de iniciales, nunca recibe listeners, asi que un producto
    sin foto jamas genera el efecto). Al mostrar, calcula la posicion
    con `getBoundingClientRect()` del thumbnail: prefiere el lado
    (derecha/izquierda) con mas espacio libre en la ventana, y hace
    clamp en los 4 bordes para que nunca quede cortado. El popup se
    ancla a `<body>` con `position:fixed` -- MISMO PATRON ya usado por
    los menus de cabecera de columna (Rango de uso/Orientación/Arnés)
    para escapar del `overflow:hidden` de `.table-shell` y el
    `overflow-x:auto` de `.table-scroll`, así que no hay riesgo de que
    quede recortado por esos contenedores. Al estar fuera del flujo del
    documento, no afecta el alto de fila ni el ancho de columnas.
  - `tabindex="0"` añadido a cada `.prod-thumb-img` para que el mismo
    popup tambien se pueda activar con teclado (foco) -- accesibilidad
    minima pedida explicitamente, sin cambiar el diseño visual.

No se toca la matriz principal, la ficha tecnica, el buscador, el
filtro de edad, el selector Slot 1/Slot 2, las ordenaciones, los datos,
los enlaces de Amazon, ni ninguna fotografia existente. La version
movil no tiene la clase `.prod-thumb-img` en ningun sitio (usa sus
propias clases `.mc-product-img`/`.mc-summary-img`/`.mc-result-thumb`),
asi que el hover-zoom no se activa nunca ahi -- y ademas la tabla PC ya
esta oculta (`display:none`) por debajo de 768px, con lo que el efecto
ni siquiera es alcanzable en movil.
...
OUT = "prototipo-comparativa-sillas-v59.html"
...
<title>PROTOTIPO v59 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v59 — <b>no es la web real</b> — misma fuente que v4-v58: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Mejora visual (26-ago-2026): al pasar el ratón (o dar foco con teclado) sobre la fotografía de un producto en la <b>tabla de escritorio (PC)</b>, aparece una versión ampliada (~200px) flotante junto a la miniatura, sin agrandar la fila ni mover columnas. Solo afecta a los productos con foto real — los que muestran iniciales no tienen ningún efecto. Miniatura, datos y resto del diseño sin cambios.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v59.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
