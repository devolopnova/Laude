#!/usr/bin/env python3
"""
build_comparativa_prototipo_v37.py

Evolucion sobre v36 (ver tools/build_comparativa_prototipo_v36.py) --
implementa UNICAMENTE la "FASE 2" del responsive movil acordada con el
usuario (18-ago-2026): matriz de comparacion de caracteristicas debajo
de las 2 tarjetas VS de la Fase 1.

Estructura: grid de 3 columnas (CARACTERÍSTICA | SILLITA A | SILLITA B),
NO una lista vertical -- cada fila son 3 <div> hijos consecutivos
(.mc-feat-label + 2x .mc-feat-val) dentro de un contenedor CSS Grid
(.mc-features), asi que las 2 columnas de valores quedan siempre
alineadas horizontalmente entre filas.

7 filas, en el orden pedido: Precio, Normativa, Rango de uso, ISOFIX,
Orientación, 360º, Arnés. build_mobile_feature_values() calcula el
valor de cada fila para un producto reutilizando EXACTAMENTE los mismos
campos/funciones que ya usa build_row() para la tabla de PC
(campo()/fmt_precio()/valor_badge()/rango_uso_html()/
orientacion_compacto_tabla()/arnes_compacto_tabla()) -- ningun dato
escrito a mano, ninguna segunda fuente de informacion; si mas adelante
cambian los 2 productos de DEMO_COMPARE_ASINS, la matriz se recalcula
sola. "Sin especificar"/"Revisar"/"—" se representan con las mismas
clases CSS que ya usa PC (cell-unspecified/cell-revisar/GUION) -- ningun
texto nuevo.

Unica excepcion CSS necesaria: dentro de la matriz movil (columnas mas
estrechas que en PC), Rango de uso permite salto de linea en vez de
recortar con ellipsis (selector `.mc-feat-val .rango-primary` mas
especifico que `.rango-primary` sola, asi que nunca afecta a la tabla
de PC).

No se toca la Fase 1 (tarjetas VS, imagenes, CSS ya existente) salvo
sustituir el contenedor `#mc-features` vacio por su contenido real. No
se modifica la tabla de PC, el dataset, las ordenaciones, la ficha
tecnica de PC ni los botones/enlaces de Amazon. Sin resumen, ganador,
badges ni conclusiones todavia -- eso queda para una fase posterior.

Uso:
    python tools/build_comparativa_prototipo_v37.py
"""
...
OUT = "prototipo-comparativa-sillas-v37.html"
...
<title>PROTOTIPO v37 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v37 — <b>no es la web real</b> — misma fuente que v4-v36: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Responsive móvil, fase 2 (18-ago-2026): debajo de las tarjetas VS, nueva <b>matriz de características</b> (Precio, Normativa, Rango de uso, ISOFIX, Orientación, 360º, Arnés) en formato tabla — no lista vertical — con los valores de SILLITA A y SILLITA B alineados. Mismo dataset y misma lógica de presentación que la tabla PC. Sin resumen ni ganador todavía. Redimensiona a &lt;768px para verla.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v37.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
