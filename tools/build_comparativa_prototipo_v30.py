#!/usr/bin/env python3
"""
build_comparativa_prototipo_v30.py

Evolucion sobre v29 (ver tools/build_comparativa_prototipo_v29.py) --
implementa UNICAMENTE el punto 7.14 acordado con el usuario
(16-ago-2026), con dos cambios:

  1. Alineacion/centrado de "Rango de uso": verificado en v29 (ya
     cumplia) -- las 4 lineas ya estaban alineadas exactamente a la
     misma izquierda (display:block, mismo left en las 4, confirmado
     con getBoundingClientRect) y el bloque ya quedaba centrado
     verticalmente como unidad (.rango-block, punto 7.11). No hizo
     falta ningun cambio de CSS adicional para este punto.
  2. Eliminacion COMPLETA del selector "Ordenar por" (<select
     id="sortSelect"> + su <div class="sort-toolbar">): ya no existe
     en el HTML ni en el CSS. La ordenacion sigue disponible desde los
     iconos/menus de cabecera (punto 7.1), que ahora son la UNICA
     interfaz de ordenacion.

     Refactor tecnico (para que la cabecera siga funcionando sin el
     <select> como "bus" de comunicacion entre setupSort() y la
     integracion de cabecera): ambos bloques de JS se fusionan en un
     unico scope; el estado "criterio activo" pasa de select.value a
     una variable `currentSortKey`; triggerSort(key) llama directamente
     a applySort(key) + updateIndicators(key) en vez de simular un
     cambio en el <select>. La logica de comparacion (compareRows) y
     los 25 criterios NO cambian ni una linea.

     AVISO (confirmado con el usuario antes de implementar): el
     selector cubria 2 criterios sin icono propio en cabecera --
     "Nº de valoraciones: mayor/menor" -- que quedan sin punto de
     entrada en la interfaz en esta version (el punto 2 de este ajuste
     pedia explicitamente no tocar los iconos/funcionalidad actuales
     de cabecera, asi que no se les añadio un icono nuevo).

No se modifica ningun dato del dataset, ningun JSON, ninguna otra
columna (Producto/Precio/Valoración/Normativa/ISOFIX/Orientación/360º/
Arnés/Acciones), la ficha tecnica, los botones Amazon, los enlaces/
ASIN, ni la logica de los 25 criterios de ordenacion. No se toca
amazon_import.py ni ningun archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v30.py
"""
...
OUT = "prototipo-comparativa-sillas-v30.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v30 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v30 — <b>no es la web real</b> — misma fuente que v4-v29: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajuste sobre v29 (punto 7.14, 16-ago-2026): <b>Rango de uso</b> ya cumplía la alineación/centrado pedidos (verificado, sin cambios de CSS). Se elimina por completo el selector <b>"Ordenar por"</b> — la ordenación sigue disponible únicamente desde los iconos/menús de cabecera (punto 7.1). El motor de ordenación (25 criterios) no cambia; solo se refactorizó su punto de entrada para no depender de un &lt;select&gt;.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v30.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
