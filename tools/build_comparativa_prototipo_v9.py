#!/usr/bin/env python3
"""
build_comparativa_prototipo_v9.py

Evolucion sobre v8 (ver tools/build_comparativa_prototipo_v8.py) --
MISMA fuente de datos, MISMO diseno base. Tres cambios:

1. La cabecera de columna "Puntos de anclaje" pasa a llamarse "Puntos
   anclaje".
2. En esa columna, en la tabla principal, se elimina el matiz entre
   parentesis del valor de arnes (p.ej. "Cinturón del vehículo (3
   puntos)" -> "Cinturón del vehículo"; "Arnés de 5 puntos (76-105cm) /
   cinturón de 3 puntos (100-150cm)" -> "Arnés de 5 puntos / cinturón de
   3 puntos") -- solo presentacion, el dato real no cambia y el texto
   integro sigue disponible en el title (tooltip) de la celda.
3. El campo "Arnés" vuelve a aparecer en la ficha tecnica desplegable
   (con el texto COMPLETO, incluido el matiz entre parentesis que ya no
   se ve en la tabla principal) -- decision tomada junto al usuario:
   tabla = vistazo rapido y limpio, ficha = detalle completo. Ya no es
   una duplicacion identica (la tabla muestra la version compacta, la
   ficha la version completa), asi que no rompe el criterio de "no
   duplicar" que se aplico con Giratoria 360º en v3.
...
Uso:
    python tools/build_comparativa_prototipo_v9.py
"""
...
OUT = "prototipo-comparativa-sillas-v9.html"
...
DETAIL_FIELDS_PRINCIPALES = [
    ...
    # "Arnés" vuelve a la ficha (v9) con el texto COMPLETO, incluido el
    # matiz entre parentesis -- en la tabla principal ("Puntos anclaje")
    # ese matiz se quita para que quede limpio (ver arnes_compacto).
    # No es duplicacion identica: tabla = version corta, ficha = version
    # completa.
    ("Arnés", "arnes"),
]
...
def build_row(p, idx):
    ...
    <tr class="main-row" data-target="{panel_id}">
      ...
      {celda_principal(arnes, "col-anclaje", arnes_compacto)}
      <td class="col-action">
...
COLUMN_HEADERS = [
    ...
    ("360º", "col-360"),
    ("Puntos anclaje", "col-anclaje"),
    ("", "col-action"),
]

# "Rango de uso" ahora lleva hasta 4 lineas (altura/edad/grupo/peso), se
# mantiene ancho generoso. Nueva columna "Puntos de anclaje" (campo
# arnes, antes solo en la ficha tecnica) necesita espacio para textos
# como "Cinturón del vehículo" o "Arnés de 5 puntos (76-105cm) / ...".
COLUMN_WIDTHS = [250, 90, 100, 190, 60, 130, 60, 170, 140]
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v9 — Comparativa sillitas de coche</title>
...
td.col-normativa{{font-size:12.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:100px;}}
...
td.col-orientacion{{font-size:13px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:130px;}}

td.col-anclaje{{font-size:12.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:170px;}}
...
<div class="proto-banner">⚠ Prototipo v9 — <b>no es la web real</b> — misma fuente que v4-v8: tools/output/auditoria_30_candidatos.json (dataset maestro consolidado 13-ago-2026, con la corrección de arnés de Nania Bogota aplicada el 14-ago-2026)</div>
<p class="proto-note">
  Cambios sobre v8: la columna "Puntos de anclaje" pasa a llamarse <b>"Puntos anclaje"</b>. En esa columna, en la tabla principal, se quita el matiz entre paréntesis del arnés (p.ej. "Cinturón del vehículo (3 puntos)" → "Cinturón del vehículo") para que quede limpia — el texto íntegro sigue en el tooltip. El campo <b>Arnés</b> vuelve a aparecer en la ficha técnica, esta vez con el texto completo (incluido el matiz) — tabla = vistazo rápido, ficha = detalle completo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v9.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v9] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}}`);
