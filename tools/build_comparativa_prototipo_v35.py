#!/usr/bin/env python3
"""
build_comparativa_prototipo_v35.py

Evolucion sobre v34 (ver tools/build_comparativa_prototipo_v34.py) --
implementa UNICAMENTE la "RESPONSIVE MÓVIL — PRIMERA FASE" acordada con
el usuario (18-ago-2026): estructura visual y funcional basica de
comparativa de 2 sillitas enfrentadas (VS) para movil, SIN tocar nada
de la version PC.

Que se añade (todo ADITIVO -- nueva funcion Python + nuevas clases CSS
+ un nuevo bloque HTML, ningun cambio en build_row/build_header_cell/
compareRows/el resto del <style> ya existente):

  - build_mobile_compare(productos): construye la tarjeta VS de 2
    productos usando el MISMO dataset y los MISMOS helpers de formato
    (fmt_precio/fmt_valoracion/fmt_n_val) que ya usa build_row() para
    la tabla de PC -- ningun dato inventado ni duplicado a mano.
  - DEMO_COMPARE_ASINS: 2 ASIN de ejemplo (Cybex Solution S2 i-Fix,
    Maxi-Cosi Tanza), cualquiera de los 30 del dataset serviria --
    arquitectura lista para que una fase futura permita elegir que 2
    comparar.
  - Imagenes REALES: el dataset JSON no tenia ningun campo de imagen
    todavia (no habia ninguna imagen real disponible en el proyecto
    para estas sillas), asi que se descargaron las fotos reales de la
    propia ficha de Amazon de esos 2 productos con el mismo pipeline
    que ya usa tools/amazon_sillas_coche.py (descarga + recuadre
    600x600 sin deformar), guardadas en images/sillas-coche/. Ninguna
    imagen generada ni de stock externo.
  - CSS nuevo (.mobile-compare/.mc-*): oculto por defecto; un unico
    media query `@media (max-width:768px)` intercambia
    `.table-shell{{display:none}}` <-> `.mobile-compare{{display:block}}`
    -- en escritorio la tabla PC se sigue viendo exactamente igual, en
    movil desaparece y aparece esta comparativa (nunca un
    overflow-x:scroll de la tabla).
  - Contenedor `#mc-features` vacio y preparado (Fase 2 pendiente:
    matriz de caracteristicas -- Precio/Normativa/Rango de uso/Altura/
    Grupo/Peso/ISOFIX/Orientación/360º/Arnés/Ficha técnica).

No se modifica ningun dato del dataset, ningun JSON, ninguna columna
de PC, la tabla de escritorio, los 25 criterios de ordenacion, el menu
de Valoración, la ficha tecnica, los botones/enlaces Amazon, el
centrado vertical, los anchos de columna ni el pulido estetico de v34.
No se toca amazon_import.py ni ningun archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v35.py
"""
...
OUT = "prototipo-comparativa-sillas-v35.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v35 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v35 — <b>no es la web real</b> — misma fuente que v4-v34: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Responsive móvil, fase 1 (18-ago-2026): nueva comparativa de <b>2 sillitas enfrentadas (VS)</b> específica para móvil (≤768px) — sustituye a la tabla PC solo en ese breakpoint, que en escritorio sigue exactamente igual. Datos e imágenes reales del mismo dataset (Cybex Solution S2 i-Fix vs. Maxi-Cosi Tanza, de ejemplo). Debajo, contenedor preparado para la matriz de características (fase 2, pendiente). Redimensiona la ventana a &lt;768px para verla.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v35.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
