#!/usr/bin/env python3
"""
build_comparativa_prototipo_v40.py

Evolucion sobre v39 (ver tools/build_comparativa_prototipo_v39.py) --
CORRECCION V39.1 acordada con el usuario (18-ago-2026): el buscador
PRINCIPAL de "Comparar 2 sillitas" (el de fuera del panel) no filtraba
al escribir.

Causa raiz: solo tenia un listener de `focus`, y ese listener
comprobaba si habia algun slot VACIO para decidir a que ranura abrir el
panel -- pero el estado inicial de la pagina ya trae las 2 sillitas de
ejemplo elegidas (Cybex/Maxi-Cosi), asi que `slotKey` salia `null` y el
codigo simplemente le quitaba el foco al campo (`els.topSearch.blur()`)
sin abrir nada. El usuario podia escribir todo lo que quisiera, que no
pasaba nada.

Cambio (unico): el buscador principal ahora reacciona a `focus` Y a
`input` (cada tecla), con `pickTargetSlot()` decidiendo la ranura de
forma robusta (la primera vacia; si las 2 ya estan ocupadas, "Sillita
2" por defecto) en vez de bloquearse cuando no hay ninguna vacia. Cada
pulsacion llama a `renderResults()` -- la MISMA funcion de filtrado que
ya usaba (y sigue usando) el buscador del panel, sin logica duplicada.
Tambien se ajusta el texto de "sin resultados" a "No hemos encontrado
ninguna sillita." (antes decia otra cosa).

No se toca nada mas: ni la salvaguarda de excluir la sillita ya elegida
en el otro slot, ni la tarjeta VS, ni la matriz de caracteristicas, ni
la ficha tecnica, ni la tabla de PC, ni el dataset, ni ninguna imagen.

Uso:
    python tools/build_comparativa_prototipo_v40.py
"""
...
OUT = "prototipo-comparativa-sillas-v40.html"
...
<title>PROTOTIPO v40 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v40 — <b>no es la web real</b> — misma fuente que v4-v39: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Corrección v39.1 (18-ago-2026): el <b>buscador principal</b> de "Comparar 2 sillitas" no filtraba al escribir (se bloqueaba porque las 2 ranuras ya venían ocupadas por defecto). Ahora funciona con cada tecla, usando la misma función de filtrado que el buscador del panel. Redimensiona a &lt;768px para probarlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v40.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function setupMobileSelector() {{
  ...
  // Correccion v39.1 (18-ago-2026): el buscador principal de "Comparar
  // 2 sillitas" solo abria el panel si HABIA un slot vacio -- pero el
  // estado inicial ya trae las 2 sillitas de ejemplo elegidas, asi que
  // en la practica nunca hacia nada (se limitaba a perder el foco). El
  // buscador principal ahora SIEMPRE funciona, escriba lo que escriba
  // el usuario y esten o no las 2 sillitas ya elegidas: si no hay panel
  // abierto lo abre (sobre la 1ª ranura vacia, o sobre "Sillita 2" si
  // las 2 ya estan ocupadas), y cada tecla vuelve a llamar a
  // renderResults() -- la MISMA funcion de filtrado que ya usa el
  // buscador del panel, sin logica duplicada.
