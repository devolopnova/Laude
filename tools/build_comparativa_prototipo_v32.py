#!/usr/bin/env python3
"""
build_comparativa_prototipo_v32.py

Evolucion sobre v31 (ver tools/build_comparativa_prototipo_v31.py) --
implementa UNICAMENTE el Ajuste 7.17 acordado con el usuario
(18-ago-2026): corrige el SENTIDO del desempate por nº de valoraciones
en la ordenacion de "Valoración".

En v31 (Ajuste 7.16) el desempate por nº de valoraciones era SIEMPRE
de mayor a menor, sin importar el sentido elegido para la puntuacion.
El Ajuste 7.17 especifica una regla distinta: el desempate debe usar
el MISMO sentido que el criterio principal --

  - Valoración mayor -> menor: en empate, mayor nº de valoraciones
    primero.
  - Valoración menor -> mayor: en empate, menor nº de valoraciones
    primero.

Cambio unico: en compareRows(), la rama 'valoracion-desc'/'valoracion-asc'
pasa de `-1 * (na - nb)` (fijo) a `dir * (na - nb)` (mismo signo que la
puntuacion). El resto de esa rama (valores nulos -> "Sin especificar"
al final, doble empate -> orden original estable) no cambia. El ciclo
de 3 estados de la cabecera (descendente/ascendente/sin ordenar) y el
resto de los 25 criterios de ordenacion tampoco cambian.

No se modifica ningun dato del dataset, ningun JSON, ninguna columna
(Producto/Precio/Normativa/Rango de uso/ISOFIX/Orientación/360º/Arnés/
Acciones), la ficha tecnica, los botones Amazon, los enlaces/ASIN, el
centrado vertical/horizontal, los estilos generales/colores/anchos de
columna, el comportamiento responsive, ni el resto de los 25 criterios
de ordenacion. No se toca amazon_import.py ni ningun archivo de
produccion.

Uso:
    python tools/build_comparativa_prototipo_v32.py
"""
...
OUT = "prototipo-comparativa-sillas-v32.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v32 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v32 — <b>no es la web real</b> — misma fuente que v4-v31: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajuste 7.17 (18-ago-2026): la ordenación de <b>Valoración</b> desempata por nº de valoraciones en el MISMO sentido que la puntuación (mayor→menor: más valoraciones primero; menor→mayor: menos valoraciones primero) — antes el desempate era siempre de mayor a menor sin importar el sentido elegido.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v32.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function setupSort() {{
  ...
  function compareRows(a, b, key) {{
    ...
    if (key === 'valoracion-desc' || key === 'valoracion-asc') {{
      ...
      // Empate en puntuacion -> desempate por nº de valoraciones, en el
      // MISMO sentido que el criterio principal (Ajuste 7.17, 18-ago-2026):
      // mayor->menor usa mayor nº de valoraciones primero; menor->mayor
      // usa menor nº de valoraciones primero.
