#!/usr/bin/env python3
"""
build_comparativa_prototipo_v50.py

Evolucion sobre v49 (ver tools/build_comparativa_prototipo_v49.py) --
CORRECCION acordada con el usuario (18-ago-2026): tras seleccionar una
edad en "Filtrar por edad", el campo "Buscar por marca o modelo" se
quedaba visualmente "activo" (borde naranja) pero dejaba de aceptar
pulsaciones reales -- no aparecia ningun producto al escribir una
marca, como si el buscador se hubiera bloqueado.

Diagnostico: v49 ya corregia que el ESTADO logico de los 2 filtros
(ageFilter/texto) se resetee correctamente en cualquier apertura del
panel (ver su docstring) -- verificado de nuevo aqui, la logica de
filtrado en si (renderResults()/passesAgeFilter()) es correcta y sigue
sin tocarse. El sintoma descrito (campo con foco visual pero sin
recibir pulsaciones) apunta a un problema de FOCO DEL NAVEGADOR movil,
no de logica de filtrado: openPanel() forzaba con
`els.panelSearch.focus()` el foco (y el teclado virtual) nada mas abrir
el panel. En algunos navegadores/webviews moviles, ese foco puesto por
JavaScript (no por un toque real del usuario), seguido de interactuar
con el `<select>` de edad (que abre su propio selector nativo del
sistema operativo), puede dejar el campo de texto en un estado de
"foco fantasma": se ve con el estilo `:focus` (borde naranja) pero no
vuelve a aceptar pulsaciones hasta recargar la pagina.

Cambio (unico): se quita esa llamada a `.focus()` dentro de
openPanel(). Sin foco automatico, cada vez que el usuario toca el
campo de busqueda es un gesto real y directo, lo cual es fiable en
cualquier navegador -- y ademas evita que el teclado tape el panel
recien abierto, un efecto tampoco deseable en movil.

No se toca: el reseteo de estado ya corregido en v49
(resetPanelState()), la logica del filtro de edad (passesAgeFilter,
AGE_FILTER_OPTIONS), la logica de asignacion de slot, el diseño de las
tarjetas de resultado, la tabla de PC, el dataset, la matriz de
caracteristicas, la ficha tecnica, la tarjeta VS, las imagenes, la
logica de comparacion ni las ordenaciones.

Uso:
    python tools/build_comparativa_prototipo_v50.py
"""
...
OUT = "prototipo-comparativa-sillas-v50.html"
...
<title>PROTOTIPO v50 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v50 — <b>no es la web real</b> — misma fuente que v4-v49: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Corrección (18-ago-2026): tras usar "Filtrar por edad", el campo "Buscar por marca o modelo" se quedaba con foco visual pero sin aceptar pulsaciones reales (probable "foco fantasma" tras el foco automático + el selector nativo de edad en móvil). Se quita el foco automático del buscador al abrir el panel — ahora cada toque del usuario es un gesto real y fiable. Redimensiona a &lt;768px para probarlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v50.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
