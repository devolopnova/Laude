#!/usr/bin/env python3
"""
build_comparativa_prototipo_v19.py

Evolucion sobre v18 (ver tools/build_comparativa_prototipo_v18.py) --
implementa UNICAMENTE el punto 7.3 acordado con el usuario (16-ago-2026):
nueva jerarquia visual de "Rango de uso" + completar dos edades.

PARTE VISUAL (jerarquia de Rango de uso):
  Antes: Altura (dato principal) > Edad > Grupo > Peso.
  Ahora: Edad (dato principal, 15px/700) > Altura (segundo nivel,
  13px/600, sigue muy visible pero ya no domina) > Grupo (12px/500) >
  Peso (11px/400, sin cambios). Clases CSS renombradas de
  rango-altura/rango-edad/rango-grupo/rango-peso a
  rango-primary/rango-secondary/rango-tertiary/rango-quaternary para
  que el nombre refleje el rol, no el campo fijo. Los margin-top entre
  lineas (3px/3px/2px) NO cambian -- misma altura total de fila que en
  v18.

PARTE DE DATOS (unica modificacion de dataset en este punto, hecha con
tools/auditoria3_edad_completar.py siguiendo el pipeline reproducible
del proyecto -- snapshot + script + diff validado):
  - Kinderkraft SAFETY FIX 3 PRO [B0FGDM2ZTS]: edad "Sin especificar"
    -> "15 meses - 12 años" (decision manual del usuario).
  - Lionelo LEVI ONE i-Size [B0D5QSYXPH]: edad "Sin especificar (por
    altura: 76-150cm)" -> "15 meses - 12 años", derivada del propio
    dataset (los otros 3 productos con la misma altura exacta
    "76-150 cm" -- Cybex Pallas G i-Size G2, KikkaBoo i-PASS, KikkaBoo
    i-FLIT -- tienen los 3 esa misma edad). Altura/grupo/peso de ambos
    productos NO se tocan. Ver informe del chat para el diff completo.

No se toca la cabecera del punto 7.1, la celda Valoración del punto
7.2, la ordenacion, ninguna otra columna, ficha tecnica, botones "Ver
en Amazon" ni ningun otro dato. No se toca amazon_import.py ni ningun
archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v19.py
"""
...
OUT = "prototipo-comparativa-sillas-v19.html"
...
def rango_uso_html(altura, edad, grupo, peso):
    ...
    visual del punto 7.3 (16-ago-2026): Edad es el dato principal,
    Altura el segundo (visible pero sin competir con Edad), Grupo y
    Peso mas discretos. Cada linea SOLO aparece si el dato existe --
    nunca se rellena el hueco con guion ni con el texto 'Sin
    especificar', simplemente no se genera esa linea."""
    lineas = []
    campos = (
        (edad, "rango-primary"),
        (altura, "rango-secondary"),
        (grupo, "rango-tertiary"),
        (peso, "rango-quaternary"),
    )
...
# quepa la palabra "valoraciones" sin desbordar. El resto de columnas
# mantiene su ancho de v11/v12.
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v19 — Comparativa sillitas de coche</title>
...
.val-star{{display:block;font-size:16px;font-weight:700;color:var(--ink);white-space:nowrap;line-height:1.2;}}
...
/* "Rango de uso" -- jerarquia del punto 7.3 (16-ago-2026): Edad (dato
   principal, mas grande) > Altura (segundo nivel, sigue muy visible y
   legible pero sin competir con Edad) > Grupo > Peso (mas discretos).
   Antes era Altura > Edad > Grupo > Peso; el orden vertical y el
   tamaño se invierten entre esos dos, Grupo/Peso no cambian. Cada
   linea solo existe en el HTML si el dato esta presente (ver
   rango_uso_html en el script) -- aqui solo se define el estilo de
   cada nivel cuando aparece. Los margin-top (3px/3px/2px) se
   mantienen igual que antes para no aumentar la altura de la fila. */
td.col-rango{{white-space:normal;max-width:190px;}}
.rango-primary{{display:block;font-size:15px;font-weight:700;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}}
.rango-secondary{{display:block;font-size:13px;font-weight:600;color:var(--ink);margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}}
.rango-tertiary{{display:block;font-size:12px;font-weight:500;color:var(--ink-soft);margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}}
.rango-quaternary{{display:block;font-size:11px;font-weight:400;color:var(--ink-soft);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}}
...
<div class="proto-banner">⚠ Prototipo v19 — <b>no es la web real</b> — misma fuente que v4-v18: tools/output/auditoria_30_candidatos.json (con las 2 edades completadas el 16-ago-2026, ver tools/auditoria3_edad_completar.py)</div>
<p class="proto-note">
  Cambio sobre v18 (punto 7.3, 16-ago-2026): nueva jerarquía en <b>Rango de uso</b> — Edad pasa a ser el dato principal (más grande), Altura al segundo nivel (visible pero sin competir), Grupo y Peso siguen discretos. Además se completaron las 2 edades que quedaban "Sin especificar": Kinderkraft SAFETY FIX 3 PRO (decisión manual) y Lionelo LEVI ONE i-Size (derivada del patrón de altura 76-150cm ya presente en el dataset). Ningún otro dato, columna ni la ordenación se han tocado.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v19.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v19] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}} | cabeceras ordenables: ${{sortableHeaders}} | items en menus de cabecera: ${{totalMenuItems}} | opciones select: ${{selectOptions}}`);
