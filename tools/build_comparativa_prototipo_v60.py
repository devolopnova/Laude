#!/usr/bin/env python3
"""
build_comparativa_prototipo_v60.py

Evolucion sobre v59 (ver tools/build_comparativa_prototipo_v59.py) --
implementa UNICAMENTE un ajuste en la columna "Puntos anclaje" (arnés)
de la tabla de escritorio (PC), a peticion del usuario (26-ago-2026):
cuando el arnés combina dos sistemas distintos unidos con "+" (caso
Chicco Everone i-Size: "Arnés de 5 puntos (40-105 cm) + cinturón de 3
puntos del vehículo (desde 100 cm)"), la tabla mostraba el "+ cinturón
3 puntos vehículo" tambien -- el usuario quiere que la tabla principal
muestre SOLO los puntos del arnés, y que el matiz del cinturón se quede
exclusivamente en la ficha tecnica (que ya lo mostraba completo, via el
mecanismo existente de arnes_tiene_matiz_oculto()).

Cambio (unico, Python -- ni CSS, ni el resto de columnas/celdas/
tipografias/datos/dataset se tocan):
  - Nueva funcion `arnes_compacto_tabla_pc()`, EXCLUSIVA de la tabla PC:
    envuelve la `arnes_compacto_tabla()` ya existente (sin tocarla) y
    ademas recorta cualquier cosa desde un "+" en adelante -- asi
    "Arnés 5 puntos + cinturón 3 puntos vehículo" queda en "Arnés 5
    puntos" para la celda de tabla.
  - `build_row()` (tabla PC) pasa a llamar a `arnes_compacto_tabla_pc`
    en vez de `arnes_compacto_tabla` para la columna "Puntos anclaje" --
    UNICO cambio en `build_row()`. El call-site movil
    (`build_mobile_feature_values()`, matriz de comparación) sigue
    llamando a `arnes_compacto_tabla()` tal cual, SIN el recorte del
    "+" -- a petición explícita del usuario, este ajuste es solo para
    PC, la matriz móvil no cambia.
  - El dato real (`campo(p, "arnes")`, usado por el tooltip de la celda
    y por la ficha tecnica) NO cambia -- sigue siendo el texto completo
    con el "+cinturón...". `arnes_tiene_matiz_oculto()` (sin tocar)
    sigue detectando la diferencia entre el texto de tabla y el
    completo, asi que la ficha tecnica PC sigue mostrando "Arnés" con
    el detalle íntegro, sin que se pierda información.
  - Solo afecta a Chicco Everone i-Size (unico producto del dataset con
    "+" en el campo arnes a 26-ago-2026); el resto de productos no
    tienen "+" en ese campo, asi que su celda/tabla no cambia.

No se toca la matriz principal, el buscador, el filtro de edad, el
selector Slot 1/Slot 2, las ordenaciones, los datos del dataset, los
enlaces de Amazon, las fotografias, ni el hover-zoom de v59. La version
movil (matriz de comparación Y ficha técnica móvil) usa
`arnes_compacto_tabla()` sin modificar y no se toca.

Uso:
    python tools/build_comparativa_prototipo_v59.py
"""
...
OUT = "prototipo-comparativa-sillas-v60.html"
...
# permita elegir que 2 comparar.
DEMO_COMPARE_ASINS = ["B0B97Z1C6M", "B0CZ47FKZL"]

# Imagenes de producto (v57: conexion en movil; v58: MISMA fuente
# reutilizada tambien por la tabla PC, 26-ago-2026). Las fotos reales de
# cada producto viven en images/sillas-coche/<ASIN>.webp (descargadas y
# validadas contra la regla de imagenes de CLAUDE.md -- ver auditoria del
# 26-ago-2026). El dataset JSON no tiene ningun campo de imagen, asi que
# la correspondencia ASIN -> archivo se resuelve aqui, a partir de lo que
# exista REALMENTE en disco en el momento de generar la pagina -- nunca
# una lista mantenida a mano, que se desincronizaria cada vez que se
# descargue/borre una imagen. Un ASIN sin archivo cae automaticamente en
# el fallback de iniciales correspondiente (build_mobile_thumb_html() en
# movil, build_row() en PC), sin necesidad de listar aqui los "sin foto".
# Un solo diccionario, calculado una vez -- ni movil ni PC duplican esta
# logica, ambos llaman a resolve_product_image(asin) mas abajo.
PRODUCT_IMAGES_DIR = "images/sillas-coche"
PRODUCT_IMAGES = {
    os.path.splitext(f)[0]: f"{PRODUCT_IMAGES_DIR}/{f}"
    for f in (os.listdir(PRODUCT_IMAGES_DIR) if os.path.isdir(PRODUCT_IMAGES_DIR) else [])
    if f.lower().endswith(".webp")
}


def resolve_product_image(asin):
    """Helper unico y reutilizable (v58): ruta de la foto real de un
    producto por ASIN, o None si no existe -- lo usan tanto build_row()
    (tabla PC) como build_mobile_thumb_html() (tarjetas moviles), sin
    logica duplicada."""
    return PRODUCT_IMAGES.get(asin)
...
def arnes_compacto(valor):
    ...
    mostrando completo. Compartida por PC y movil -- ver
    arnes_compacto_tabla_pc() para el recorte adicional exclusivo de PC
    (26-ago-2026)."""
...
def arnes_compacto_tabla_pc(valor):
    """SOLO para la tabla de escritorio (PC) -- version de
    arnes_compacto_tabla() que ADEMAS recorta un segundo sistema de
    sujeción unido con "+" (p.ej. 'Arnés 5 puntos + cinturón 3 puntos
    vehículo' -> 'Arnés 5 puntos'), a petición explícita del usuario
    (26-ago-2026): en la tabla principal solo debe verse el sistema de
    arnés en sí, nunca el "+cinturón..." -- ese matiz se queda
    exclusivamente en la ficha técnica, que ya lo muestra completo via
    arnes_tiene_matiz_oculto() (sin cambios, sigue detectando la
    diferencia contra el valor íntegro). NO se usa en móvil -- su
    matriz de comparación sigue llamando a arnes_compacto_tabla() tal
    cual, sin este recorte adicional; este ajuste es exclusivo de PC."""
    texto, matiz = arnes_compacto_tabla(valor)
    if texto in (GUION, "Revisar"):
        return texto, matiz
    recortado = re.sub(r'\s*\+.*$', '', texto).strip()
    if recortado and recortado != texto:
        return recortado, (valor or "").strip()
    return texto, matiz
...
def build_mobile_thumb_html(p, img_cls, ph_cls=""):
    """Imagen real si existe en images/sillas-coche/<ASIN>.webp
    (resolve_product_image(), ver comentario mas arriba), o si no el
    MISMO fallback que ya usa la tabla de PC para TODOS sus productos:
    un recuadro con las iniciales de la marca (iniciales()), nunca un
    placeholder generico/inventado."""
    marca = p.get("marca") or SIN_ESPECIFICAR
    modelo = p.get("modelo") or SIN_ESPECIFICAR
    alt = html_lib.escape(f"{marca} {modelo}")
    imagen = resolve_product_image(p["asin"])
    if imagen:
...
def build_mobile_vs_slot_inner(p):
    ...
    marca + modelo + valoración + nº valoraciones + precio + botón
    "Ver en Amazon →") para UN producto -- extraido a funcion propia
    (18-ago-2026, fase del selector) para poder reutilizarlo tanto en
    el render inicial en servidor (los 2 productos de ejemplo) como en
    el JSON que consume el selector movil para poder cambiar de
    sillita sin recargar la pagina -- el boton de Amazon se regenera
    automaticamente con el ASIN correcto cada vez, igual que el resto
    de la tarjeta. Misma logica exacta de siempre, ningun dato nuevo.

    Ajuste (18-ago-2026): se añade el boton "Ver en Amazon →" debajo
    del precio, reutilizando TAL CUAL la clase .btn-amazon-row -- el
    mismo boton solido/naranja que ya usa la tabla de PC, sin CSS
    nueva para el boton en si (solo un margen propio para separarlo
    del precio dentro de la tarjeta movil)."""
    marca = p.get("marca") or SIN_ESPECIFICAR
    modelo = p.get("modelo") or SIN_ESPECIFICAR
    ...
    amazon_html = (
        f'<a class="btn-amazon-row mc-product-amazon" '
        f'href="https://www.amazon.es/dp/{html_lib.escape(p["asin"])}" '
        f'target="_blank" rel="nofollow sponsored noopener">Ver en Amazon →</a>'
    )
    return f'''{thumb_html}
...
def _mc_result_field(label, *value_htmls):
    """Bloque etiqueta + valor(es) para la tarjeta de resultado del
    selector (rediseño 18-ago-2026). Cada value_html ya viene resuelto
    por _mc_rango_campo() -- si es cadena vacia (sin dato para ese
    campo), se descarta aqui; si TODOS los valores de un campo estan
    vacios, no se genera el bloque entero (ni la etiqueta se muestra
    huerfana) -- nunca se inventa un dato ni se deja una linea vacia
    con su etiqueta."""
    valores = [v for v in value_htmls if v]
    if not valores:
        return ""
    valores_html = "".join(f'<span class="mc-result-field-value">{v}</span>' for v in valores)
    return (
        f'<span class="mc-result-field">'
        f'<span class="mc-result-field-label">{html_lib.escape(label)}</span>'
        f'{valores_html}</span>'
    )
...
def build_mobile_result_card_html(p):
    ...
    (rediseño 18-ago-2026, EXCLUSIVO de esta tarjeta -- no afecta a la
    tarjeta VS ni al chip "Sillita elegida", que usan sus propias
    funciones/clases): imagen con mucho mas protagonismo (ver
    .mc-result-thumb en el CSS), informacion organizada por etiquetas
    ("Rango de uso"/"Altura"/"Grupo / peso") en vez de una sola linea
    con puntos separadores, y SIN ISOFIX (ya esta en la matriz de
    comparacion, no hace falta duplicarlo aqui -- el dato/la logica de
    ISOFIX en si no se toca en ningun otro sitio).

    Cada campo reutiliza _mc_rango_campo() -- LA MISMA funcion que ya
    usa la matriz principal para Edad/Altura/Peso/Grupo -- asi que
    "sin dato, no se muestra esa linea" es identico en los dos sitios,
    sin logica de formato nueva ni datos inventados."""
    marca = p.get("marca") or SIN_ESPECIFICAR
    modelo = p.get("modelo") or SIN_ESPECIFICAR
    precio = fmt_precio(p.get("precio")) or SIN_ESPECIFICAR
    valoracion = fmt_valoracion(p.get("valoracion"))
    ...
    campo_edad = _mc_result_field("Rango de uso", _mc_rango_campo(normalizar_recien_nacido(edad)))
    campo_altura = _mc_result_field("Altura", _mc_rango_campo(altura))
    campo_grupo_peso = _mc_result_field("Grupo / peso", _mc_rango_campo(grupo), _mc_rango_campo(peso_recomendado))
    ...
    return f'''{thumb_html}
      <span class="mc-result-body">
        ...
        {campo_grupo_peso}
        {rating_html}
        <span class="mc-result-price">{html_lib.escape(precio)}</span>
      </span>'''
...
def build_row(p, idx):
    ...
    # Foto real si existe en images/sillas-coche/<ASIN>.webp (v58), si no
    # el fallback de iniciales de siempre -- misma caja 44x44px en los
    # dos casos (.prod-thumb-img / .prod-thumb-ph), object-fit:contain
    # para no deformar ni recortar la foto.
    prod_imagen = resolve_product_image(p["asin"])
    if prod_imagen:
        prod_thumb_html = (
            f'<img class="prod-thumb-img" src="{prod_imagen}" '
            f'alt="{html_lib.escape(f"{marca} {modelo}")}" width="44" height="44" loading="lazy">'
        )
    else:
        prod_thumb_html = f'<div class="prod-thumb-ph">{iniciales(marca)}</div>'
    ...
    <tr class="main-row" data-target="{panel_id}"{sort_attrs}>
      ...
      {celda_principal(arnes, "col-anclaje", arnes_compacto_tabla_pc)}
      <td class="col-action">
...
<title>PROTOTIPO v60 — Comparativa sillitas de coche</title>
...
/* Ajuste (18-ago-2026): boton "Ver en Amazon ->" debajo del precio --
   .btn-amazon-row (definida mas abajo) ya trae su propio estilo
   solido/naranja identico al de la tabla de PC; aqui solo se fija la
   separacion respecto al precio dentro de esta tarjeta en concreto. */
.mc-product-amazon{{margin-top:10px;}}
...
/* Ajuste (18-ago-2026), SOLO movil: el texto "FICHA TÉCNICA" pasa de
   var(--ink) (gris/tinta) a var(--accent-deep) -- el MISMO naranja de
   marca que ya usan .mc-product-brand/.mc-product-price/
   .btn-amazon-row, sin crear ninguna variable/color nuevo. El icono
   (.mc-ficha-toggle-icon, mas abajo) ya usaba var(--accent-deep) desde
   siempre, asi que no necesita cambio. Fondo, borde, sombra, tamaño,
   padding, radio y el funcionamiento del acordeon no se tocan. */
.mc-ficha-toggle{{
  width:100%; display:flex; align-items:center; justify-content:space-between; gap:10px;
  background:var(--card); border:1px solid var(--line); border-radius:14px;
  box-shadow:0 2px 8px rgba(0,0,0,.05); padding:14px 16px; cursor:pointer;
  font-family:'Inter',sans-serif; font-size:13px; font-weight:700; color:var(--accent-deep);
...
/* Ajuste (18-ago-2026), SOLO ficha tecnica movil: columna de etiqueta
   92px -> 120px -- la matriz principal (.mc-features, mas arriba) NO
   se toca, sigue en 92px. El motivo es la etiqueta larga "Reposacabezas:
   altura regulable": a 92px, la palabra "Reposacabezas:" (14
   caracteres) no cabe en el ancho util de la celda (92px - 16px de
   padding = 76px) y se sale/pega al borde en vez de ajustarse; a
   120px si cabe comodamente en su propia linea, con "altura
   regulable" debajo en una segunda linea, tal como se pidio. Las
   otras 2 columnas de valores pierden 14px cada una (de 1fr a un 1fr
   mas estrecho) -- comprobado que sigue sobrando espacio de sobra
   para valores cortos como "Sí"/"No"/"No especificado"/"6,2 kg".
   overflow-wrap:break-word como red de seguridad adicional (scoped
   solo aqui) para que ninguna etiqueta, por larga que sea, pueda
   desbordar el borde de su celda. */
.mc-ficha-panel{{
  margin-top:10px; display:grid; grid-template-columns:120px 1fr 1fr;
  background:var(--card); border:1px solid var(--line); border-radius:14px;
  box-shadow:0 2px 8px rgba(0,0,0,.05); overflow:hidden;
}}
.mc-ficha-panel .mc-feat-label{{overflow-wrap:break-word;}}
...
/* Selector "Comparar 2 sillitas" (18-ago-2026) -- mismo lenguaje
   ...
   contexto (140px en la tarjeta VS, 48px en el chip "Sillita elegida",
   96px en los resultados del selector -- ver .mc-result-thumb mas
   abajo, rediseño 18-ago-2026). */
.mc-thumb-ph{{
  display:flex; align-items:center; justify-content:center;
...
.mc-summary-img.mc-thumb-ph{{font-size:14px;}}
.mc-result-thumb.mc-thumb-ph{{font-size:22px;}}
...
/* Rediseño de la tarjeta de resultado (18-ago-2026): imagen mucho mas
   grande y protagonista (align-items:flex-start para que quede
   anclada arriba, ya que el bloque de texto ahora es mas alto con los
   campos con etiqueta) + mas padding vertical para que la tarjeta
   respire con el contenido extra. */
.mc-result-card{{
  width:100%; display:flex; align-items:flex-start; gap:14px; padding:16px 10px;
...
/* Imagen protagonista (18-ago-2026): 56px -> 96px, salto claro y
   decidido (no un ligero aumento) para que la sillita se reconozca de
   un vistazo al elegir entre resultados. object-fit:contain (ya
   estaba) para no deformar ni recortar; el fallback de iniciales
   (.mc-thumb-ph) usa la MISMA clase .mc-result-thumb, asi que
   comparte automaticamente este mismo tamaño -- todas las tarjetas
   reservan el mismo espacio tengan o no imagen real. */
.mc-result-thumb{{width:96px; height:96px; flex-shrink:0; border-radius:12px; object-fit:contain;}}
.mc-result-body{{display:flex; flex-direction:column; gap:2px; min-width:0; flex:1;}}
.mc-result-head{{display:flex; flex-direction:column; margin-bottom:4px;}}
.mc-result-brand{{font-size:11px; font-weight:700; color:var(--accent-deep); text-transform:uppercase; letter-spacing:.03em;}}
.mc-result-model{{font-size:13.5px; font-weight:600; color:var(--ink);}}
/* Campos con etiqueta ("Rango de uso"/"Altura"/"Grupo / peso"),
   sustituyen a la antigua linea unica con puntos separadores -- cada
   uno ocupa su propia linea y solo aparece si esa sillita tiene el
   dato (ver _mc_result_field() en el script: sin dato, sin bloque). */
.mc-result-field{{display:flex; flex-direction:column; margin-top:5px;}}
.mc-result-field-label{{font-size:10px; font-weight:700; color:var(--ink-soft); text-transform:uppercase; letter-spacing:.03em;}}
.mc-result-field-value{{font-size:12.5px; font-weight:600; color:var(--ink); line-height:1.35;}}
.mc-result-rating{{font-size:12px; font-weight:700; color:var(--ink); margin-top:6px;}}
.mc-result-reviews{{font-size:11px; font-weight:400; color:var(--ink-soft);}}
.mc-result-price{{display:block; font-size:13.5px; font-weight:700; color:var(--ink); margin-top:4px;}}
...
/* Foto real de producto en la tabla PC (26-ago-2026, v58) -- misma caja
   44x44px que .prod-thumb-ph (arriba, sin tocar) para que la fila no se
   mueva ni un pixel entre productos con foto y sin ella. */
.prod-thumb-img{{
  width:44px;height:44px;border-radius:9px;object-fit:contain;flex-shrink:0;
  border:1px solid var(--line);background:#fff;
}}
/* Hover-zoom sobre la foto de la tabla PC (26-ago-2026, v59). Miniatura
   sin cambios (arriba); solo se añade el cursor y el popup flotante
   -- ver .prod-zoom-popup mas abajo y el JS que lo posiciona con
   position:fixed (mismo patron ya usado por los menus de cabecera de
   columna, para escapar del overflow:hidden de .table-shell /
   overflow-x:auto de .table-scroll). Las fotos son siempre cuadradas
   (pad_to_square, 600x600), asi que un popup cuadrado fijo con
   object-fit:contain nunca deforma ni recorta. */
.prod-thumb-img{{cursor:zoom-in;}}
.prod-zoom-popup{{
...
.prod-zoom-popup.is-visible{{opacity:1;transform:scale(1);}}
...
<div class="proto-banner">⚠ Prototipo v60 — <b>no es la web real</b> — misma fuente que v4-v59: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajuste (26-ago-2026): en la columna <b>"Puntos anclaje"</b> de la tabla de escritorio (PC), cuando el arnés combina dos sistemas ("Arnés 5 puntos + cinturón vehículo"), ahora solo se muestra el sistema de arnés — el "+cinturón..." se queda exclusivamente en la ficha técnica, que ya lo mostraba completo. Solo afecta a Chicco Everone i-Size. La matriz de comparación móvil no cambia.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v60.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function setupMobileSelector() {{
  ...
  function renderResults(query) {{
    ...
    if (lista.length === 0) {{
      els.panelResults.innerHTML = '<p style="padding:24px 4px;color:var(--ink-soft);font-size:13px;text-align:center;">No hemos encontrado ninguna sillita.</p>';
      return;
    }}
  ...
  function openPanel(slotKey) {{
    ...
    // Se quita el .focus() programatico (que forzaba el teclado virtual
    // nada mas abrir el panel) -- en movil puede dejar el campo en un
    // estado de "foco fantasma" que no acepta pulsaciones reales tras
    // interactuar con otros controles, y ademas el teclado tapaba el
    // panel recien abierto. El usuario toca el campo el mismo cuando
    // quiere escribir, un gesto real y siempre fiable.
  }}
...
// Hover-zoom sobre las fotos reales de la tabla PC (26-ago-2026, v59).
// IIFE propia y autocontenida -- no toca ni depende de ninguna otra
// funcion/variable del prototipo. Solo actua sobre .prod-thumb-img (las
// fotos reales); .prod-thumb-ph (iniciales) nunca recibe listeners, asi
// que un producto sin foto nunca genera el efecto. El popup se crea via
// JS y se ancla a <body> con position:fixed (mismo patron que los menus
// de cabecera de columna, mas arriba) para no quedar recortado por el
// overflow:hidden/overflow-x:auto de la tabla, y no afecta el layout
// porque esta fuera del flujo normal del documento.
(function prodThumbZoom() {{
  const thumbs = document.querySelectorAll('.prod-thumb-img');
  if (!thumbs.length) return;

  const popup = document.createElement('img');
  popup.className = 'prod-zoom-popup';
  popup.alt = '';
  ...
  function show(thumb) {{
    ...
    // Preferencia: el lado con mas espacio libre -- si el hueco a la
    // derecha ya basta para el popup, o hay mas hueco a la derecha que
    // a la izquierda, se muestra a la derecha; si no, se voltea a la
    // izquierda. Clamp final para que nunca se salga de la ventana por
    // ningun borde (arriba/abajo/izquierda/derecha).
    let left = (spaceRight >= SIZE + GAP || spaceRight >= spaceLeft)
      ? rect.right + GAP
      : rect.left - GAP - SIZE;
  ...
  thumbs.forEach(thumb => {{
    ...
    thumb.addEventListener('mouseenter', () => show(thumb));
    thumb.addEventListener('mouseleave', hide);
    thumb.addEventListener('focus', () => show(thumb));
