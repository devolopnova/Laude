#!/usr/bin/env python3
"""
build_comparativa_prototipo_v27.py

Evolucion sobre v26 (ver tools/build_comparativa_prototipo_v26.py) --
ajuste EXCLUSIVO de la ficha tecnica (16-ago-2026), a peticion del
usuario tras revisar v26: las particularidades de instalación/arnés
dejan de mostrarse como bloque independiente (titulo naranja
"PARTICULARIDAD DE INSTALACIÓN" + seccion propia con borde separador)
y pasan a ser UNA FILA MAS dentro de la misma lista de caracteristicas,
con exactamente el mismo estilo que Reclinable/Reposacabezas/etc.

Cambios:
  - Las etiquetas cambian de "Particularidad de instalación"/
    "Particularidad del arnés" a "Tipo de instalación"/"Arnés" (el
    nombre habitual del dato), y se añaden a la MISMA lista
    `ficha_campos` en vez de a un array/contenedor `particularidades`
    aparte -- ya no existe el <div class="ficha-particularidades">.
  - `detail_item()` pierde el parametro `particularidad` (ya no hay
    ningun estilo especial que aplicar) -- todas las filas usan
    siempre la misma clase `.detail-item`.
  - CSS: se eliminan las reglas `.ficha-particularidades` y
    `.detail-item--particularidad` (dead code, ya no se generan).
  - La logica de CUANDO aparecen (instalacion_particularidad() /
    arnes_tiene_matiz_oculto()) NO cambia -- mismo criterio de antes,
    solo cambia donde y como se pintan.

No se modifica ningun dato del dataset, ningun JSON, la tabla
principal, ninguna columna, la cabecera, la ordenacion, el resto de la
ficha tecnica (los 6 campos principales, "Sin especificar", el boton
"Ver en Amazon"), ni los enlaces/ASIN. No se toca amazon_import.py ni
ningun archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v27.py
"""
...
OUT = "prototipo-comparativa-sillas-v27.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v27 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v27 — <b>no es la web real</b> — misma fuente que v4-v26: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo la ficha técnica)</div>
<p class="proto-note">
  Ajuste sobre v26 (16-ago-2026): dentro de la <b>ficha técnica</b>, las particularidades de instalación/arnés dejan de ser un bloque independiente (título naranja + sección propia) y pasan a ser una fila más de la lista normal — "Tipo de instalación" y "Arnés" con exactamente el mismo estilo que Reclinable, Reposacabezas, etc. La lógica de cuándo aparecen no cambia. No se ha tocado ningún dato, el resto de la ficha, la tabla principal, la cabecera ni la ordenación.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v27.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v27] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}} | cabeceras ordenables: ${{sortableHeaders}} | items en menus de cabecera: ${{totalMenuItems}} | opciones select: ${{selectOptions}} | campos ficha (min 6x30): ${{fichaCampos}} | botones cerrar ficha: ${{fichaCloseBtns}} | bloques de particularidad sueltos (deben ser 0): ${{fichaBloquesSueltos}}`);
