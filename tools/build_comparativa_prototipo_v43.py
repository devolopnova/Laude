#!/usr/bin/env python3
"""
build_comparativa_prototipo_v43.py

Evolucion sobre v42 (ver tools/build_comparativa_prototipo_v42.py) --
implementa UNICAMENTE los "AJUSTES EN LA TABLA DE COMPARACIÓN" (matriz
de caracteristicas movil, la de #mc-features) acordados con el usuario
(18-ago-2026). Dos cambios, nada mas:

  1. Se elimina la fila "Precio" de la matriz -- ya se muestra en la
     tarjeta VS superior (debajo de la valoracion), duplicarlo no
     aportaba nada. La matriz pasa de 7 a 9 filas (ver punto 2) y
     ahora empieza directamente en "Normativa". build_mobile_feature_
     values() ya no calcula "precio".

  2. "Rango de uso" deja de ser UNA fila que apilaba Edad/Altura/Grupo/
     Peso dentro de la misma celda (con altura variable segun cuantas
     lineas tuviera cada sillita -- eso desalineaba Sillita 1 y
     Sillita 2 entre si, el bug reportado) y pasa a 4 FILAS
     independientes del grid, una por atributo, en el orden pedido:
     Edad, Altura, Peso, Grupo (distinto del orden interno de
     rango_uso_html, que sigue siendo Edad/Altura/Grupo/Peso -- esa
     funcion no se toca, sigue usandose tal cual en la tabla de PC y en
     las tarjetas de resultado del buscador). Al ser filas de grid
     normales, la altura de cada fila la comparten las 2 columnas
     automaticamente (igual que ya pasa con Normativa/ISOFIX/etc.) --
     ya no se puede desalinear.

     Campos sin dato en estas 4 filas nuevas: a peticion EXPLICITA del
     usuario, la celda se deja COMPLETAMENTE VACIA (sin guion, sin
     "Sin especificar", sin ningun texto) -- distinto del resto de la
     app, que normalmente muestra "—"/"Sin especificar". Nueva funcion
     _mc_rango_campo() para esto, usada SOLO en estas 4 filas. La
     celda (`<div class="mc-feat-val">`) se sigue generando vacia (no
     se omite), para no perder la fila ni desalinear con la otra
     columna. "Revisar" SI se sigue mostrando (no es una ausencia de
     dato).

No se toca el diseño visual (cero clases CSS nuevas o modificadas --
las celdas vacias/con texto plano ya se veian bien con el CSS
existente de .mc-feat-val, igual que Normativa/ISOFIX ya usaban antes),
la tabla de PC, el dataset, el orden de Normativa/ISOFIX/Orientación/
360º/Arnés, las tarjetas VS superiores, la ficha tecnica, los
buscadores/selector de sillitas ni ninguna otra funcionalidad.

Uso:
    python tools/build_comparativa_prototipo_v43.py
"""
...
OUT = "prototipo-comparativa-sillas-v43.html"
...
def build_mobile_feature_values(p):
    ...
    (valor_badge/orientacion_compacto_tabla/arnes_compacto_tabla) que
    ...
    usuario para estos 4 campos, distinto del resto de la app); Edad
    normaliza "Recién nacido" -> "0" igual que ya hace rango_uso_html()
    para la tabla de PC."""
...
<title>PROTOTIPO v43 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v43 — <b>no es la web real</b> — misma fuente que v4-v42: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajustes en la matriz de comparación (18-ago-2026): se elimina la fila <b>Precio</b> (ya está en la tarjeta VS) y <b>Rango de uso</b> pasa a 4 filas independientes (Edad/Altura/Peso/Grupo) para que Sillita 1 y Sillita 2 queden siempre alineadas — sin dato, la celda queda vacía, nunca con guion o "Sin especificar". Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v43.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
