#!/usr/bin/env python3
"""
build_comparativa_prototipo_v39.py

Evolucion sobre v38 (ver tools/build_comparativa_prototipo_v38.py) --
implementa UNICAMENTE el "SELECTOR PARA COMPARAR 2 SILLITAS" acordado
con el usuario (18-ago-2026): buscador/selector movil que permite
elegir CUALQUIER par de los 30 productos del dataset para comparar, en
vez de estar fijado a los 2 ASIN de ejemplo.

ARQUITECTURA (clave para que "no se duplique logica de datos"): todo el
HTML dependiente de datos (tarjeta VS, matriz de caracteristicas, ficha
tecnica, tarjeta de resultado de busqueda, chip de sillita elegida) se
sigue generando en Python, con las MISMAS funciones de siempre
(campo/valor_badge/rango_uso_html/build_mobile_feature_values/
build_mobile_ficha_values) -- para los 30 productos, no solo 2. Ese
HTML ya renderizado se embebe en la pagina como JSON
(`const MOBILE_PRODUCTS = [...]`, ver build_mobile_selector()). El
JavaScript del selector NO recalcula ni reinterpreta ningun dato: solo
decide QUE 2 productos estan elegidos y coloca sus fragmentos ya
renderizados en el sitio correspondiente (data-slot="a"/"b" en
.mc-product, data-feat/data-ficha en las celdas de la matriz/ficha).

Estado inicial: exactamente los mismos 2 productos de siempre
(DEMO_COMPARE_ASINS/DEMO_ASIN_A/DEMO_ASIN_B) -- la pagina se ve
identica a v38 hasta que el usuario cambie algo, no hay parpadeo de
re-render al cargar.

Imagenes: solo 2 de los 30 productos tienen imagen real descargada
(DEMO_COMPARE_IMAGES, sin cambios). Para el resto, tanto en la tarjeta
VS como en el buscador, se usa build_mobile_thumb_html() -- el MISMO
fallback que ya usa la tabla de PC para TODOS sus productos (recuadro
con iniciales de la marca, iniciales()) -- no se genera ningun
placeholder nuevo ni se descargan imagenes nuevas en esta fase.

Nuevas piezas:
  - build_mobile_thumb_html/build_mobile_vs_slot_inner/
    build_mobile_summary_html/build_mobile_result_card_html/
    build_mobile_product_payload: fragmentos HTML por producto.
  - build_mobile_selector(): seccion "Comparar 2 sillitas" + panel de
    busqueda + el <script> con MOBILE_PRODUCTS, van ANTES de la
    tarjeta VS dentro de .mobile-compare.
  - JS setupMobileSelector(): busca (subcadena sobre marca+modelo),
    selecciona/quita sillita 1 y 2, activa "Comparar sillitas" solo con
    2 elegidas, y al pulsar redibuja tarjeta VS + matriz + ficha con
    los fragmentos ya renderizados y hace scroll hasta la comparativa.

No se toca la tabla de PC, el dataset, las ordenaciones, el menu de
Valoración, la ficha tecnica de PC, los botones/enlaces de Amazon, la
matriz principal ni la ficha tecnica movil ya aprobadas (mismas
funciones de calculo, solo se les añaden atributos data-* para que el
selector las pueda localizar).

Uso:
    python tools/build_comparativa_prototipo_v39.py
"""
...
OUT = "prototipo-comparativa-sillas-v39.html"
...
def build_mobile_feature_values(p):
    ...
    UN producto. Reutiliza exactamente los mismos campos (campo()) y
    las mismas funciones de compactacion/presentacion
    (valor_badge/rango_uso_html/orientacion_compacto_tabla/
    arnes_compacto_tabla) que ya usa build_row() para la tabla de PC --
    ningun dato nuevo, ninguna segunda fuente de informacion. El
    "Sin especificar"/"Revisar"/"—" se representan exactamente igual
    que en PC (mismas clases CSS, mismo texto)."""
    ...
    return {
        ...
        "rango": rango_uso_html(altura, edad, grupo, peso_recomendado),
        "isofix": valor_badge(isofix),
        "orientacion": _mc_valor_compacto(orientacion, orientacion_compacto_tabla),
        "giro360": valor_badge(giro_360, sin_especificar_como_guion=True),
...
def _mc_medida(valor):
    ...
    Si/No, es una medida) -- mismo criterio Sin especificar/Revisar que
    el resto del prototipo (cell-unspecified/cell-revisar), el valor no
    se reduce a Si/No porque no tiene sentido para un peso."""
    v = (valor or "").strip()
    if es_sin_especificar(v):
        return f'<span class="cell-unspecified">{SIN_ESPECIFICAR}</span>'
    if es_revisar(v):
...
def build_mobile_ficha_values(p):
    ...
    Funda lavable) reutilizan valor_badge() -- la MISMA funcion que ya
    usa la tabla de PC para ISOFIX/360º, que ya distingue exactamente
    los 3 estados pedidos: 'Sí' (dato confirma que existe), 'No' (dato
    confirma explicitamente que no existe -- nunca inventado, solo
    aparece si el dataset ya trae un 'No' literal) y 'Sin especificar'
    (sin dato) en gris discreto (clase txt-plain--unk, ya usada en toda
    la app) -- nunca se convierte una ausencia en 'No'."""
    ...
    return {
        "reposacabezas": valor_badge(reposacabezas),
        "peso_silla": _mc_medida(peso_silla),
        "proteccion_lateral": valor_badge(proteccion_lateral),
        "reclinable": valor_badge(reclinable),
        "funda_lavable": valor_badge(funda_lavable),
    }
...
<title>PROTOTIPO v39 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v39 — <b>no es la web real</b> — misma fuente que v4-v38: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Responsive móvil (18-ago-2026): nuevo <b>selector "Comparar 2 sillitas"</b> — busca entre los 30 productos reales y elige cualquier par; la tarjeta VS, la matriz y la ficha técnica se actualizan con los fragmentos ya renderizados por Python (sin recalcular nada en JS). Empieza con las mismas 2 sillitas de siempre. Redimensiona a &lt;768px para verla.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v39.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function setupMobileSelector() {{
  ...
  function renderSlot(slotKey) {{
    ...
    const p = byAsin[asin];
    el.innerHTML = `<div class="mc-slot-filled">${{p.summaryHtml}}<button type="button" class="mc-slot-remove" data-remove="${{slotKey}}" aria-label="Quitar sillita">✕</button></div>`;
  }}
  ...
  function renderResults(query) {{
    ...
    const otherAsin = pickingSlot === 'a' ? selected[1] : selected[0];
    const lista = MOBILE_PRODUCTS.filter(p => {{
      if (p.asin === otherAsin) return false;
      if (!q) return true;
      return (p.marca + ' ' + p.modelo).toLowerCase().includes(q);
    }});
    if (lista.length === 0) {{
      els.panelResults.innerHTML = '<p style="padding:24px 4px;color:var(--ink-soft);font-size:13px;text-align:center;">Sin resultados para esa búsqueda.</p>';
      return;
    }}
  ...
  if (els.topSearch) {{
    els.topSearch.addEventListener('focus', () => {{
      const slotKey = !selected[0] ? 'a' : (!selected[1] ? 'b' : null);
      if (!slotKey) {{ els.topSearch.blur(); return; }}
      openPanel(slotKey);
  ...
  document.addEventListener('click', (e) => {{
    ...
    const resultCard = e.target.closest('.mc-result-card');
    if (resultCard && !els.panel.hasAttribute('hidden')) {{
      selected[slotIndex(pickingSlot)] = resultCard.dataset.asin;
      renderSlot(pickingSlot);
      updateCompareBtn();
      closePanel();
