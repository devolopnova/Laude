#!/usr/bin/env python3
"""
build_comparativa_prototipo_v45.py

Evolucion sobre v44 (ver tools/build_comparativa_prototipo_v44.py) --
implementa UNICAMENTE "UNIFICAR EL TEXTO DE DATOS NO DISPONIBLES"
acordado con el usuario (18-ago-2026): en v44 la matriz principal movil
ya decia "No especificado" (ver su docstring), pero la ficha tecnica
movil seguia diciendo "Sin especificar" -- dos expresiones distintas
para lo mismo. Ahora las dos dicen "No especificado".

Cambio (unico): en build_mobile_ficha_values() (los 5 campos de la
ficha tecnica movil -- Reposacabezas/Peso de la silla/Protección
lateral/Reclinable/Funda lavable), los 4 campos Sí/No pasan de
valor_badge() (texto "Sin especificar" para el caso sin dato) a
_mc_valor_badge() (la version movil ya creada en v44, "No
especificado") -- misma funcion que ya usa el resto de la matriz
principal para ISOFIX/360º, sin crear nada nuevo. _mc_medida() (el
campo de medida "Peso de la silla", ya exclusivo de movil) cambia su
texto directamente de "Sin especificar" a "No especificado".

NO se toca: Rango de uso (Edad/Altura/Peso/Grupo) -- sigue dejando la
celda completamente vacia sin dato, regla explicita distinta que el
usuario confirmo mantener; valor_badge() en si (PC sigue mostrando
"Sin especificar" en ISOFIX/360º exactamente igual que siempre); el
ISOFIX de las tarjetas de resultado del buscador/selector (esa es zona
de "buscadores/selector", que el usuario ha pedido repetidamente no
tocar salvo que se pida explicitamente sobre ella) ni ninguna otra
funcionalidad, diseño, dato o el orden de caracteristicas.

Uso:
    python tools/build_comparativa_prototipo_v45.py
"""
...
OUT = "prototipo-comparativa-sillas-v45.html"
...
<title>PROTOTIPO v45 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v45 — <b>no es la web real</b> — misma fuente que v4-v44: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Unificación de texto (18-ago-2026): la <b>ficha técnica móvil</b> decía "Sin especificar" mientras la matriz principal ya decía "No especificado" — ahora las dos usan la misma expresión, <b>"No especificado"</b>, en toda la versión móvil. Rango de uso (Edad/Altura/Peso/Grupo) sigue dejando la celda vacía, sin cambios. Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v45.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
