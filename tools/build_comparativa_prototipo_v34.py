#!/usr/bin/env python3
"""
build_comparativa_prototipo_v34.py

Evolucion sobre v33 (ver tools/build_comparativa_prototipo_v33.py) --
implementa UNICAMENTE el "PULIDO ESTETICO FINAL" acordado con el
usuario (18-ago-2026): ajustes EXCLUSIVAMENTE visuales (CSS) para que
la tabla se vea mas premium/ligera/editorial, sin tocar ninguna
funcionalidad, dato, columna ni logica de ordenacion.

Cambios (todos en el <style>, ninguno en el HTML/JS de la tabla):

  1. Altura de fila: tbody td padding vertical 14px -> 11px (mas
     compacta, sin apretar -- Rango de uso, la celda mas alta con sus
     4 lineas posibles, se sigue leyendo comoda). Padding horizontal
     (12px) NO se toca -- varias columnas tienen ancho fijo estrecho
     (55px ISOFIX/360º, 84px Valoración) y ensancharlo arriesgaria
     desbordar texto.
  2. Separadores entre filas mas sutiles: nueva variable --line-row
     (color-mix de --line al 55%, mas claro) usada SOLO en los
     border-bottom de tbody td -- el resto de usos de --line (borde de
     .table-shell, borde bajo la cabecera, bordes de los menus de
     ordenacion, etc.) no cambia.
  3. Cabecera mas ligera: padding 19px 14px -> 15px 14px, en thead th Y
     en .th-sort-btn a la vez (el boton debe seguir rellenando todo el
     <th>). Nombres de columna, indicadores de ordenacion y
     funcionalidad de cabecera intactos.
  4. Producto: gap entre MARCA y MODELO 2px -> 3px (aire minimo,
     tipografias/colores sin cambios).

Todo lo demas queda exactamente igual: Valoración (tamaño, menu de 2
criterios), Rango de uso (jerarquia, espaciado interno, centrado
vertical), botones (Ver en Amazon / Ficha técnica, sin rediseño),
anchos de columna, los 25 criterios de ordenacion, centrado vertical
global (celda_principal/.cell-block/.txt-plain), ficha tecnica,
enlaces/ASIN de Amazon.

No se modifica ningun dato del dataset, ningun JSON, ninguna columna,
la ficha tecnica, los botones Amazon, los enlaces/ASIN, el centrado
vertical, los anchos de columna, el comportamiento responsive, ni
ningun criterio de ordenacion. No se toca amazon_import.py ni ningun
archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v34.py
"""
...
OUT = "prototipo-comparativa-sillas-v34.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v34 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v34 — <b>no es la web real</b> — misma fuente que v4-v33: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Pulido estético final (18-ago-2026): filas más compactas (padding vertical 14px→11px), separadores entre filas más sutiles, cabecera más ligera (padding 19px→15px) y un pequeño respiro entre marca y modelo — solo CSS, sin tocar datos, columnas ni ordenación.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v34.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
