#!/usr/bin/env python3
"""
build_comparativa_prototipo_v56.py

Evolucion sobre v55 (ver tools/build_comparativa_prototipo_v55.py) --
implementa UNICAMENTE el ajuste de anchura/wrapping de la primera
columna de la FICHA TÉCNICA movil acordado con el usuario (18-ago-2026).

Problema: la etiqueta "Reposacabezas: altura regulable" (v46) no cabia
en el ancho util de la celda a 92px (92px - 16px de padding = 76px),
asi que la palabra "Reposacabezas:" (14 caracteres) se salia/pegaba al
borde en vez de ajustarse limpiamente en su propia linea.

Cambio (unico, CSS, SOLO en `.mc-ficha-panel` -- la matriz principal
`.mc-features` NO se toca, sigue en 92px):
  - grid-template-columns pasa de `92px 1fr 1fr` a `120px 1fr 1fr`,
    suficiente para que "Reposacabezas:" quepa en su propia linea y
    "altura regulable" en la siguiente.
  - `.mc-ficha-panel .mc-feat-label{{overflow-wrap:break-word;}}` como
    red de seguridad adicional (scoped solo a la ficha tecnica) para
    que ninguna etiqueta pueda desbordar el borde de su celda.

Las 2 columnas de valores pierden 14px cada una (comprobado que sigue
sobrando espacio de sobra para valores cortos como "Sí"/"No"/"No
especificado"/pesos en kg). El color/peso/tamaño/alineacion vertical de
`.mc-feat-label` en si NO se tocan (sigue en var(--ink), 12px, 700,
centrado) -- la etiqueta sigue en el mismo tono oscuro de siempre, el
gris (txt-plain--unk/cell-unspecified) sigue exclusivo de los valores
"No especificado".

No se toca la matriz principal, la tarjeta VS, el boton Amazon, el
buscador, el selector de sillitas, el contenido/los datos de la ficha
tecnica ni la tabla de PC.

Uso:
    python tools/build_comparativa_prototipo_v56.py
"""
...
OUT = "prototipo-comparativa-sillas-v56.html"
...
<title>PROTOTIPO v56 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v56 — <b>no es la web real</b> — misma fuente que v4-v55: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajuste de anchura (18-ago-2026): la columna de etiqueta de la <b>Ficha técnica</b> (móvil) pasa de 92px a 120px para que etiquetas largas como "Reposacabezas: altura regulable" quepan sin salirse del margen en pantallas de 390-430px, envolviendo en dos líneas cuando hace falta — la matriz principal de comparación no se toca, sigue en 92px. Mismo estilo tipográfico, sin gris (el gris sigue siendo exclusivo de "No especificado"). Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v56.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
