#!/usr/bin/env python3
"""
build_comparativa_prototipo_v17.py

Evolucion sobre v16 (ver tools/build_comparativa_prototipo_v16.py).
v16 añadio 4 criterios de ordenacion por caracteristica (ISOFIX/360º/
Orientación/Arnés) solo accesibles desde el <select> "Ordenar por",
dejando sus cabeceras como texto plano a proposito ("se valorara en el
repaso estetico posterior"). Tras revisar v16, el usuario pidio
explicitamente adelantar esa integracion visual YA: esta version
añade el indicador ↕ clicable tambien en esas 4 cabeceras, con el
mismo patron ya usado en Precio/Valoración/Rango de uso (punto 7.1):

  - ISOFIX / 360º: clic directo en la cabecera, alternan sus 2
    prioridades (Sí primero / No primero).
  - Orientación / Arnés: abren un menu compacto con sus 3 prioridades
    (mismas claves que ya usaba el <select>, sin inventar ninguna).

No se toca la logica de ordenacion (compareRows sigue exactamente
igual que en v16) -- solo se añaden mas puntos de entrada en la
cabecera que disparan el mismo <select>/evento 'change' de siempre.
Solo puede haber un menu de cabecera abierto a la vez (Rango de uso /
Orientación / Arnés se excluyen entre si).
...
Uso:
    python tools/build_comparativa_prototipo_v17.py
"""
...
OUT = "prototipo-comparativa-sillas-v17.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v17 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v17 — <b>no es la web real</b> — misma fuente que v4-v16: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo cabecera)</div>
<p class="proto-note">
  Cambio sobre v16 (16-ago-2026): las cabeceras de ISOFIX, 360º, Orientación y Arnés incorporan ahora el indicador <b>↕</b> clicable, igual que Precio/Valoración/Rango de uso — ISOFIX/360º alternan directamente sus 2 prioridades, Orientación/Arnés abren un menú compacto con sus 3. Solo un menú de cabecera puede estar abierto a la vez. La lógica de ordenación (los 11 criterios, "Sin especificar" al final, estabilidad) no se ha tocado — son solo nuevos puntos de entrada al mismo selector.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v17.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v17] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}} | cabeceras ordenables: ${{sortableHeaders}} | items en menus de cabecera: ${{totalMenuItems}} | opciones select: ${{selectOptions}}`);
