#!/usr/bin/env python3
"""
build_comparativa_prototipo_v41.py

Evolucion sobre v40 (ver tools/build_comparativa_prototipo_v40.py) --
implementa UNICAMENTE la "CORRECCION DEL COMPORTAMIENTO DEL SELECTOR DE
SILLITAS" acordada con el usuario (18-ago-2026): con las 2 sillitas ya
elegidas, usar el buscador sustituia SIEMPRE la de "Sillita 2" sin
preguntar. Ahora el usuario decide que slot cambiar.

Comportamiento nuevo:
  - 0 o 1 sillitas elegidas: sigue igual que en v40 (la ranura vacia es
    inequivoca, se resuelve sola) -- resolveTargetSlot() sustituye a
    pickTargetSlot() con la misma logica para estos 2 casos.
  - 2 sillitas elegidas: resolveTargetSlot() YA NO elige por su cuenta
    -- devuelve null salvo que el usuario haya activado un slot
    explicitamente tocandolo. El buscador principal, en ese caso, no
    abre nada hasta que se toca un slot (nunca sustituye Slot 2 "porque
    si").
  - Tocar la tarjeta de una sillita YA elegida ahora tambien la activa
    para cambiarla (antes solo funcionaba en los slots vacios, via
    data-pick) -- se añade `data-activate="a"/"b"` al slot lleno
    (renderSlot()) y una rama nueva en el listener de click que llama
    a openPanel() igual que data-pick. Tocar el boton ✕ (data-remove)
    sigue funcionando igual y tiene prioridad si el clic cae en el
    boton (se comprueba antes en el listener).
  - Al elegir una sillita nueva, solo se toca el indice del slot activo
    en el array `selected` y solo se vuelve a renderizar ese slot -- el
    otro producto permanece intacto (esto ya funcionaba asi, no
    cambia).
  - Quitar una sillita con ✕ deja un unico slot ocupado -> vuelve al
    caso "1 elegida", inequivoco, se resuelve solo de nuevo.
  - La salvaguarda de no poder elegir el mismo ASIN en ambos slots
    sigue intacta (excluye `selected[otro]` en renderResults(), que ya
    usa el pickingSlot correcto en todos los casos).

No se toca el diseño visual (ninguna clase CSS nueva ni modificada), la
tabla de PC, el dataset, la matriz de caracteristicas, la ficha
tecnica, las imagenes ni ninguna funcion de calculo existente -- solo
la logica de asignacion de slot dentro de setupMobileSelector().

Uso:
    python tools/build_comparativa_prototipo_v41.py
"""
...
OUT = "prototipo-comparativa-sillas-v41.html"
...
<title>PROTOTIPO v41 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v41 — <b>no es la web real</b> — misma fuente que v4-v40: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Corrección (18-ago-2026): con las 2 sillitas ya elegidas, el buscador sustituía siempre "Sillita 2" sin preguntar. Ahora, en ese caso, hay que <b>tocar la tarjeta</b> de la sillita que quieres cambiar (funciona igual para un slot vacío o ya elegido) — con 0 o 1 elegidas sigue resolviéndose solo, como antes. Redimensiona a &lt;768px para probarlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v41.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function setupMobileSelector() {{
  ...
  // Correccion (18-ago-2026): el buscador principal de "Comparar 2
  // sillitas" ya funcionaba (v39.1) para 0 o 1 sillitas elegidas --
  // ahi la ranura vacia es inequivoca (solo puede ser una), asi que se
  // sigue resolviendo sola. El problema nuevo era el caso de las 2 YA
  // elegidas: antes se sustituia Slot 2 SIEMPRE, sin preguntar. Ahora,
  // con las 2 llenas, resolveTargetSlot() NUNCA elige por su cuenta --
  // solo usa el slot que el usuario haya activado explicitamente
  // tocando su tarjeta (data-pick en vacio / data-activate en lleno,
  // ver mas abajo); si no ha tocado ninguno, el buscador principal no
  // abre nada (no se adivina que sillita quiere cambiar).
