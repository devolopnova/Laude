#!/usr/bin/env python3
"""
build_comparativa_prototipo_v20.py

Evolucion sobre v19 (ver tools/build_comparativa_prototipo_v19.py) --
CORRIGE la jerarquia visual de "Rango de uso" del punto 7.3, a peticion
explicita del usuario tras revisar v19: en v19 Edad quedaba con un
tamaño mayor que Altura (15px vs 13px), y el usuario pidio que Edad y
Altura tengan EXACTAMENTE la misma jerarquia visual (mismo tamaño,
mismo peso, misma importancia) -- Edad va primero porque es el primer
dato del bloque, no porque deba destacar mas. Ademas se reduce de 4
tamaños de texto distintos a solo 2 niveles, para bajar el ruido
visual:

  - Nivel principal (Edad + Altura): 14px, font-weight 700, mismo color.
  - Nivel secundario (Grupo + Peso): 12px, font-weight 500, mismo color.

El espaciado entre lineas pasa de margin-top fijo por campo a
selectores CSS de hermano adyacente (.rango-primary + .rango-primary,
etc.), para que la separacion siga siendo correcta aunque falte
cualquiera de los 4 campos (ej. si falta Edad, Altura pasa a ser la
primera linea y no necesita margen superior, sin tener que duplicar
logica en Python).

El resto del punto 7.3 (las dos edades completadas -- Kinderkraft
SAFETY FIX 3 PRO y Lionelo LEVI ONE i-Size, ver
tools/auditoria3_edad_completar.py) NO se toca en esta version: mismo
dataset que v19, ningun cambio adicional de datos.
...
Uso:
    python tools/build_comparativa_prototipo_v20.py
"""
...
OUT = "prototipo-comparativa-sillas-v20.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v20 — Comparativa sillitas de coche</title>
...
/* "Rango de uso" -- jerarquia del punto 7.3 CORREGIDA (16-ago-2026):
   SOLO 2 niveles (antes eran 4 tamaños distintos, demasiado ruido
   visual). Edad y Altura son datos PRINCIPALES -- mismo tamaño, mismo
   peso, misma clase; Edad va primero por ser el primer dato del
   bloque, no por destacar mas. Grupo y Peso son datos SECUNDARIOS,
   tambien con estilo identico entre si. El espaciado entre lineas usa
   selectores de hermano adyacente (en vez de margin-top fijo por
   clase) para que funcione igual de bien tanto si estan los 4 campos
   como si falta alguno: separacion pequeña DENTRO del mismo nivel
   (Edad->Altura, o Grupo->Peso), separacion algo mayor en el cambio de
   nivel (de Edad/Altura a Grupo/Peso) para marcar el salto de
   jerarquia sin aumentar mucho la altura total de la fila. */
td.col-rango{{white-space:normal;max-width:190px;}}
.rango-primary{{display:block;font-size:14px;font-weight:700;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}}
.rango-secondary{{display:block;font-size:12px;font-weight:500;color:var(--ink-soft);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}}
...
<div class="proto-banner">⚠ Prototipo v20 — <b>no es la web real</b> — misma fuente que v4-v19: tools/output/auditoria_30_candidatos.json (mismo dataset que v19, sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Corrección sobre v19 (punto 7.3, 16-ago-2026): la jerarquía de <b>Rango de uso</b> pasa de 4 tamaños distintos a solo 2 niveles — Edad y Altura son ahora datos principales con idéntico tamaño/peso (Edad va primero por ser el primer dato del bloque, no por destacar más), Grupo y Peso son ambos secundarios con estilo idéntico entre sí. Ningún otro dato, columna, ficha técnica ni la ordenación se han tocado.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v20.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v20] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}} | cabeceras ordenables: ${{sortableHeaders}} | items en menus de cabecera: ${{totalMenuItems}} | opciones select: ${{selectOptions}}`);
