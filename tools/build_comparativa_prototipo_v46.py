#!/usr/bin/env python3
"""
build_comparativa_prototipo_v46.py

Evolucion sobre v45 (ver tools/build_comparativa_prototipo_v45.py) --
implementa UNICAMENTE el cambio de nombre de caracteristica acordado
con el usuario (18-ago-2026), EXCLUSIVO de movil: la etiqueta
"Reposacabezas" de la ficha tecnica movil pasa a
"Reposacabezas: altura regulable", para dejar claro que el dato
compara si es regulable en altura, no simplemente si la sillita tiene
reposacabezas.

Cambio (unico): la clave `("Reposacabezas", "reposacabezas")` de
MOBILE_FICHA_ROWS pasa a `("Reposacabezas: altura regulable",
"reposacabezas")`. El campo del dataset (`reposacabezas`) y sus
valores (Sí/No/No especificado, via _mc_valor_badge() desde v45) NO
cambian -- solo el texto de la etiqueta.

Diseño: no hizo falta ningun cambio de CSS -- .mc-feat-label (la misma
clase que ya usan todas las etiquetas de la matriz principal y de la
ficha tecnica) nunca ha tenido `white-space:nowrap` ni `text-overflow:
ellipsis`, asi que ya envolvia el texto de forma natural en varias
lineas cuando no cabe en los 92px de la columna; el tamaño de fuente
(12px) tampoco se toca.

NO se toca la etiqueta "Reposacabezas" de la tabla de PC (usa su propia
lista FICHA_CAMPOS_PRINCIPALES, independiente de MOBILE_FICHA_ROWS,
sin tocar), ningun otro nombre de caracteristica, los valores/datos
originales ni ninguna otra funcionalidad o diseño.

Uso:
    python tools/build_comparativa_prototipo_v46.py
"""
...
OUT = "prototipo-comparativa-sillas-v46.html"
...
<title>PROTOTIPO v46 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v46 — <b>no es la web real</b> — misma fuente que v4-v45: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Cambio de nombre (18-ago-2026): en la ficha técnica móvil, <b>"Reposacabezas"</b> pasa a <b>"Reposacabezas: altura regulable"</b> — deja claro que se compara si es regulable en altura, no solo si tiene reposacabezas. El valor (Sí/No/No especificado) no cambia. Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v46.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
