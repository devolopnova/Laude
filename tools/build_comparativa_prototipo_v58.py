#!/usr/bin/env python3
"""
build_comparativa_prototipo_v58.py

Evolucion sobre v57 (ver tools/build_comparativa_prototipo_v57.py) --
implementa UNICAMENTE la conexion de las fotografias de producto ya
descargadas en images/sillas-coche/ con la TABLA DE ESCRITORIO (PC),
que hasta ahora mostraba siempre el recuadro de iniciales de marca sin
comprobar nunca si habia una foto real disponible (26-ago-2026, fase
siguiente tras cerrar la version movil).

Cambio (unico, Python, SOLO en la celda de producto de `build_row()` --
ni CSS, ni tamanos/columnas/orden/tipografias/colores/espaciados/
responsive de la tabla PC, ni el dataset, ni las funciones de calculo/
formateo existentes se tocan):
  - El diccionario `MOBILE_PRODUCT_IMAGES` (v57) se renombra a
    `PRODUCT_IMAGES` -- ya no es exclusivo de movil, es la MISMA fuente
    de verdad (escaneo de images/sillas-coche/ por ASIN) que ahora
    consultan tanto la tabla PC como el selector movil. Ninguna logica
    duplicada: un solo diccionario, calculado una sola vez.
  - Nuevo helper `resolve_product_image(asin)` (una linea, envuelve
    `PRODUCT_IMAGES.get(asin)`) para que tanto `build_mobile_thumb_html()`
    como `build_row()` resuelvan la imagen de la misma forma.
  - `build_row()`: la celda de producto (`<div class="prod-thumb-ph">`,
    siempre el recuadro de iniciales) pasa a comprobar
    `resolve_product_image(p["asin"])` primero -- si existe la foto,
    renderiza `<img class="prod-thumb-img" ...>` (misma caja 44x44px,
    mismo border-radius, `object-fit:contain` para no deformar/recortar,
    mismo hueco en el layout); si no existe, se queda EXACTAMENTE el
    `<div class="prod-thumb-ph">{iniciales(marca)}</div>` de siempre, sin
    ningun cambio. La clase `.prod-thumb-img` es CSS nuevo (no toca
    `.prod-thumb-ph`), con las mismas medidas exactas para que el diseño
    de la fila no se mueva ni un pixel entre productos con foto y sin
    ella.
  - Correspondencia exclusivamente por ASIN (nombre de archivo
    `<ASIN>.webp`) -- nunca por marca/nombre aproximado, así que no hay
    riesgo de imagen cruzada entre productos.
...
ni los enlaces de Amazon. La version movil sigue exactamente igual --
`build_mobile_thumb_html()` resuelve la imagen con el mismo helper y el
mismo diccionario de siempre, solo cambio de nombre de variable.

Uso:
    python tools/build_comparativa_prototipo_v57.py
"""
...
OUT = "prototipo-comparativa-sillas-v58.html"
...
<title>PROTOTIPO v58 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v58 — <b>no es la web real</b> — misma fuente que v4-v57: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Fase siguiente (26-ago-2026): ahora también la <b>tabla de escritorio (PC)</b> muestra la fotografía real de <b>images/sillas-coche/&lt;ASIN&gt;.webp</b> cuando existe, en vez del recuadro de iniciales — la versión móvil ya lo hacía desde v57. Correspondencia exclusivamente por ASIN, misma caja 44×44px con o sin foto, mismo diseño/columnas/orden/tipografías de la tabla. Los productos sin foto validada siguen mostrando iniciales.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v58.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
