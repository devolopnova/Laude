#!/usr/bin/env python3
"""
build_comparativa_prototipo_v15.py

Evolucion sobre v14 (ver tools/build_comparativa_prototipo_v14.py) --
implementa UNICAMENTE el punto 7.1 acordado con el usuario (16-ago-2026):
rediseño estetico de la CABECERA de la tabla (tipografia, peso, tamaño,
espaciado, alineacion, altura, indicadores de ordenacion), a partir de
una referencia visual (imagen aportada por el usuario, usada solo como
guia de direccion estetica -- ninguno de sus datos/textos se usa).

Cambios:
  1. Cabecera con mas aire (padding vertical mayor), tipografia un poco
     mas grande y con mas letter-spacing, mismo fondo diferenciado
     (#F4F2EE) que ya existia.
  2. Alineacion por columna: Producto a la izquierda, el resto
     centrado (antes todo alineado a la izquierda).
  3. Indicadores de ordenacion (↕) SOLO en Precio, Valoración y Rango
     de uso -- las columnas que ya son criterios de ordenacion. El
     resto de cabeceras (Normativa/ISOFIX/Orientación/360º/Arnés/
     Acciones) no llevan indicador, tal como se pidio.
  4. Precio y Valoración son clicables directamente en la cabecera
     (alternan asc/desc). Rango de uso abre un pequeño menu con los 4
     sub-criterios (Altura/Edad/Grupo/Peso), cada uno con sus dos
     sentidos, para no sobrecargar la cabecera con 8 indicadores.
  5. El selector "Ordenar por" NO se elimina -- sigue exactamente
     igual, visible y funcional, como via alternativa/segura mientras
     se valida la integracion en cabecera (pedido explicito del
     usuario).

IMPORTANTE: la logica de ordenacion (applySort/compareRows, los 14
criterios, la regla de "Sin especificar" al final, la estabilidad) NO
se toca. Los nuevos controles de cabecera son solo una interfaz
adicional que dispara el MISMO selector ya existente (cambian su
`value` y disparan el mismo evento 'change' que ya escuchaba el
selector), por eso no hay ninguna logica de ordenacion nueva ni
duplicada.

No se modifica ningun dato del dataset, ningun JSON, ninguna columna,
ni filas/celdas/botones/ficha tecnica. No se toca amazon_import.py ni
ningun archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v15.py
"""
...
OUT = "prototipo-comparativa-sillas-v15.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v15 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v15 — <b>no es la web real</b> — misma fuente que v4-v14: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo estética de cabecera)</div>
<p class="proto-note">
  Cambio sobre v14 (punto 7.1, 16-ago-2026): rediseño de la <b>cabecera</b> de la tabla (tipografía, espaciado, alineación por columna) a partir de una referencia visual. Indicadores de ordenación (↕) solo en Precio/Valoración/Rango de uso — las 3 columnas que son criterio de orden — clicables directamente. El selector "Ordenar por" se mantiene intacto y funcional como vía alternativa. La lógica de ordenación (los 14 criterios, "Sin especificar" al final, estabilidad) no se ha tocado. Ninguna columna, dato, botón ni ficha técnica cambia respecto a v14.
</p>
...
<div class="sort-toolbar">
  ...
  <select id="sortSelect">
    ...
    <optgroup label="Nº de valoraciones">
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v15.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v15] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}} | cabeceras ordenables: ${{sortableHeaders}} | items menu Rango de uso: ${{rangoMenuItems}}`);
