#!/usr/bin/env python3
"""
build_comparativa_prototipo_v52.py

REVERSION acordada con el usuario (18-ago-2026): el "FILTRO POR EDAD"
del selector movil (introducido en v48, con correcciones en v49/v50/
v51) se quita por completo -- pese a las correcciones sucesivas, el
usuario siguio reportando que el buscador quedaba bloqueado/sin
refrescar al combinarlo con el filtro de edad, y decidio revertirlo en
vez de seguir iterando.

Este archivo parte de v47 (tools/build_comparativa_prototipo_v47.py),
la ULTIMA version anterior a que existiera el filtro de edad -- no es
un "deshacer" quirurgico de v48-v51, es volver al estado limpio y ya
validado de v47. Se conserva de forma independiente una UNICA mejora
de v50 que sigue siendo valida sin el filtro de edad: quitar el
`.focus()` programatico de openPanel() (evita que el teclado virtual
se abra solo y tape el panel recien abierto, y el posible "foco
fantasma" en algunos navegadores moviles) -- una mejora de foco/UX
independiente del filtro de edad, sin motivo para perderla.

Confirmado: cero referencias a AGE_FILTER_OPTIONS/ageFilter/
mc-selector-age en todo el archivo (verificado con grep antes de
generar este docstring). El selector de sillitas vuelve a tener
EXCLUSIVAMENTE "Buscar por marca o modelo" (sin el desplegable
"Filtrar por edad" debajo), igual que en v47.

No se toca: la logica de asignacion de slot, el diseño de las tarjetas
de resultado, la tabla de PC, el dataset, la matriz de caracteristicas,
la ficha tecnica, la tarjeta VS (incluido el color de precio de v47),
las imagenes, la logica de comparacion ni las ordenaciones.

Uso:
    python tools/build_comparativa_prototipo_v52.py
"""
...
OUT = "prototipo-comparativa-sillas-v52.html"
...
<title>PROTOTIPO v52 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v52 — <b>no es la web real</b> — misma fuente que v4-v51: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Reversión (18-ago-2026): se quita por completo el <b>filtro por edad</b> del selector móvil (introducido en v48) — el selector vuelve a tener únicamente "Buscar por marca o modelo", como en v47. Se conserva la mejora de foco de v50 (sin auto-focus al abrir el panel). Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v52.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
