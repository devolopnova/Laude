#!/usr/bin/env python3
"""
build_comparativa_prototipo_v42.py

Evolucion sobre v41 (ver tools/build_comparativa_prototipo_v41.py) --
implementa UNICAMENTE la correccion del BUSCADOR GENERAL acordada con
el usuario (18-ago-2026). En v41, con las 2 sillitas ya elegidas, el
buscador general ("Buscar por marca o modelo" de fuera de los slots)
no hacia NADA hasta que se tocaba un slot -- funcionalmente correcto
(nunca sustituia sin preguntar) pero el usuario queria poder buscar
directamente y decidir el slot DESPUES de elegir el producto, no antes.

Comportamiento nuevo del buscador general:
  1. 0 sillitas elegidas -> el resultado elegido va a Sillita 1 (sin
     cambios, ya funcionaba).
  2. 1 sillita elegida -> el resultado elegido va al slot vacio (sin
     cambios, ya funcionaba).
  3. 2 sillitas elegidas -> el buscador general SI se abre y deja
     buscar/elegir con normalidad; tras elegir, aparece un paso
     "¿Qué sillita quieres sustituir?" con las 2 sillitas actuales
     (Sillita 1 -- [actual] / Sillita 2 -- [actual]) y SOLO al tocar
     una de las dos se aplica el cambio, sobre ese slot unicamente.

El buscador de cada slot (tocar Sillita 1 / Sillita 2, vacia o llena)
NO cambia: sigue fijando el slot de antemano (openPanel(slotKey)), asi
que su busqueda modifica siempre y solo ese slot, sin paso de
confirmacion -- exactamente igual que en v41.

Piezas nuevas: `pendingAsin` (producto elegido por el buscador general
en espera de confirmacion), `renderSlotChoice(asin)` (la pregunta de 2
opciones) y `applyChoice(slotKey, asin)` (aplica el cambio, comun a
ambos flujos). `renderResults()` ahora excluye las 2 sillitas actuales
cuando el buscador esta en modo "general" (pickingSlot === null,
sentido nuevo de ese valor) en vez de solo la del otro slot.

renderSlotChoice() y su etiqueta "Sillita 1/2" NO usan ninguna clase
CSS nueva -- reutilizan tal cual .mc-result-card (la misma tarjeta de
resultado) y estilos en linea con los tokens de color ya existentes,
mismo patron que ya usaba el mensaje "No hemos encontrado ninguna
sillita." No se toca el diseño visual del resto, la tabla de PC, el
dataset, la matriz de caracteristicas, la ficha tecnica, las imagenes
ni ninguna funcion de calculo existente.

Uso:
    python tools/build_comparativa_prototipo_v42.py
"""
...
OUT = "prototipo-comparativa-sillas-v42.html"
...
<title>PROTOTIPO v42 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v42 — <b>no es la web real</b> — misma fuente que v4-v41: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Corrección (18-ago-2026): el <b>buscador general</b> ahora funciona con las 2 sillitas ya elegidas — deja buscar y elegir con normalidad, y tras elegir pregunta "¿Qué sillita quieres sustituir?" en vez de bloquearse. El buscador de cada slot (tocar Sillita 1/2) sigue igual, sin preguntar. Redimensiona a &lt;768px para probarlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v42.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
