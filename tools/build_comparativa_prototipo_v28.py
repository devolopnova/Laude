#!/usr/bin/env python3
"""
build_comparativa_prototipo_v28.py

Evolucion sobre v27 (ver tools/build_comparativa_prototipo_v27.py) --
ajuste EXCLUSIVO de nomenclatura (16-ago-2026): la categoria "A favor
de la marcha" de la columna Orientación pasa a mostrarse como "A
favor" en toda la interfaz, para evitar que se trunque visualmente
("A favor de la ma...").

Cambios (SOLO presentacion/etiquetas, nunca el dato ni la logica):
  - Nueva funcion `orientacion_compacto_tabla()` (mismo patron que
    `arnes_compacto_tabla`): recorta "A favor de la marcha" -> "A
    favor" en la celda de la tabla principal; el tooltip conserva el
    texto completo. "Ambas" y "A contramarcha" no cambian.
  - Menu de cabecera "Orientación" (ORIENTACION_MENU_ITEMS) y opcion
    del <select> "Ordenar por": "A favor de la marcha primero" -> "A
    favor primero".
  - `parse_orientacion_categoria()` (la funcion que clasifica el dato
    para la ordenacion) NO se toca -- sigue comparando contra "A favor
    de la marcha" tal cual esta en el dataset. El dato interno del
    producto sigue siendo exactamente "A favor de la marcha".

No se modifica ningun dato del dataset, ningun JSON, ninguna otra
columna, la ficha tecnica, botones Amazon, enlaces, ASIN, la
ordenacion de otros campos, tamaños de fuente ni anchuras de columna.
No se toca amazon_import.py ni ningun archivo de produccion.

Uso:
    python tools/build_comparativa_prototipo_v28.py
"""
...
OUT = "prototipo-comparativa-sillas-v28.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v28 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v28 — <b>no es la web real</b> — misma fuente que v4-v27: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo la etiqueta de "A favor de la marcha")</div>
<p class="proto-note">
  Ajuste sobre v27 (16-ago-2026): la categoría <b>"A favor de la marcha"</b> de Orientación se muestra ahora como <b>"A favor"</b> en la tabla y en el selector/menú de ordenación, para evitar que se trunque ("A favor de la ma..."). "Ambas" y "A contramarcha" no cambian. El dato interno y la lógica de ordenación siguen usando "A favor de la marcha" tal cual — es solo un cambio de etiqueta visible.
</p>
...
<div class="sort-toolbar">
  ...
  <select id="sortSelect">
    ...
    <optgroup label="Características">
      ...
      <option value="orientacion-favor">Orientación: A favor primero</option>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v28.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
