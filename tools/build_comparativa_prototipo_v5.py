#!/usr/bin/env python3
"""
build_comparativa_prototipo_v5.py

Evolucion de presentacion sobre el prototipo v4 (ver
tools/build_comparativa_prototipo_v4.py) -- MISMA fuente de datos
(tools/output/auditoria_30_candidatos.json), MISMA estructura de tabla,
MISMO diseno base (tipografias, colores, tarjetas, botones, mecanismo de
desplegable). Solo cambia la capa de presentacion:

1. "Sin especificar" en la tabla principal se muestra como "—" (el valor
   real del dataset sigue siendo "Sin especificar"; solo cambia el texto
   visible). El tooltip conserva "Sin especificar" al pasar el raton.
2. Se ocultan del frontend las notas de procedencia/confianza que trae
   el dataset entre parentesis (p.ej. "(Amazon, confianza media)",
   "(confirmado por fabricante)", "(resuelto manualmente)") -- ver
   limpiar_procedencia() mas abajo. El dato interno del JSON NO se toca,
   esto es puramente de presentacion.
3. La ficha tecnica desplegable sigue exactamente la misma regla que v4
   (confirmado -> se muestra; "Sin especificar" -> se oculta el campo
   entero; "Revisar" -> se muestra como "Revisar" con el detalle en el
   tooltip, nunca se resuelve aqui) y ahora agrupa visualmente los campos
   en "Datos principales" / "Caracteristicas" (separacion ligera, sin
   tarjetas dobles).
...
Uso:
    python tools/build_comparativa_prototipo_v5.py
"""
...
OUT = "prototipo-comparativa-sillas-v5.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v5 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v5 — <b>no es la web real</b> — misma fuente que v4: tools/output/auditoria_30_candidatos.json (dataset maestro consolidado 13-ago-2026)</div>
<p class="proto-note">
  Evolución de presentación sobre v4, mismo dataset y misma estructura de columnas. Cambios: <code>Sin especificar</code> se muestra como <code>—</code> en la tabla principal (pasa el ratón para ver "Sin especificar"); las notas de procedencia/confianza del dataset ("Amazon, confianza media", "confirmado por fabricante", "resuelto manualmente"...) se ocultan de la interfaz — siguen intactas en el JSON. La ficha técnica agrupa los campos en "Datos principales" / "Características" y sigue ocultando por completo cualquier campo "Sin especificar".
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v5.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v5] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}}`);
