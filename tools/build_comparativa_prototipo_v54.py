#!/usr/bin/env python3
"""
build_comparativa_prototipo_v54.py

Evolucion sobre v53 (ver tools/build_comparativa_prototipo_v53.py) --
implementa UNICAMENTE el boton "Ver en Amazon →" en las tarjetas VS
moviles acordado con el usuario (18-ago-2026).

Cambio (unico, en build_mobile_vs_slot_inner() -- la misma funcion que
ya genera imagen/marca/modelo/valoración/precio de la tarjeta VS, tanto
para el render inicial en servidor como para el JSON que consume el
selector movil al cambiar de sillita): se añade un enlace justo debajo
del precio, reutilizando TAL CUAL la clase `.btn-amazon-row` -- el
MISMO boton solido/naranja que ya usa la tabla de PC, sin CSS nueva
para el boton en si (solo `.mc-product-amazon{{margin-top:10px}}` para
separarlo del precio dentro de esta tarjeta). El enlace usa
`https://www.amazon.es/dp/{{ASIN}}`, el mismo patron ya usado en el
resto del prototipo (tabla de PC, ficha tecnica).

Como el boton se genera dentro de build_mobile_vs_slot_inner(), se
recalcula automaticamente con el ASIN correcto cada vez que se
renderiza esa funcion -- tanto en el HTML inicial (los 2 productos de
ejemplo) como cuando el usuario cambia de sillita desde el selector
(el JS ya sustituye el HTML completo de `.mc-product[data-slot]` por
el `vsSlotHtml` actualizado de cada producto, sin cambios en esa
logica), asi que el enlace de Amazon siempre corresponde al producto
que se este mostrando en ese momento.

Orden final de la tarjeta (jerarquia pedida): imagen -> marca -> modelo
-> valoración -> nº de valoraciones -> precio -> "Ver en Amazon →".

No se toca: el buscador, el filtro por edad (no existe, se elimino en
v52), la logica de asignacion de slot (Slot1/Slot2, sustitucion del
activo, exclusion del mismo ASIN), la matriz de caracteristicas, la
ficha tecnica, la tabla de PC, el dataset, las imagenes ya existentes,
la logica de comparacion ni las ordenaciones. Las tarjetas de resultado
del selector (rediseñadas en v53) tampoco se tocan.

Uso:
    python tools/build_comparativa_prototipo_v54.py
"""
...
OUT = "prototipo-comparativa-sillas-v54.html"
...
<title>PROTOTIPO v54 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v54 — <b>no es la web real</b> — misma fuente que v4-v53: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Nuevo botón (18-ago-2026): cada tarjeta VS móvil incluye ahora <b>"Ver en Amazon →"</b> debajo del precio, con el mismo estilo sólido/naranja que ya usa la tabla PC — se regenera solo con el ASIN correcto al cambiar de sillita desde el selector. Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v54.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
