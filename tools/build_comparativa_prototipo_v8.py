#!/usr/bin/env python3
"""
build_comparativa_prototipo_v8.py

Evolucion sobre v7 (ver tools/build_comparativa_prototipo_v7.py) --
MISMA fuente de datos, MISMO diseno base. Tres cambios:

1. "Rango de uso" incorpora la EDAD justo debajo de la altura: ahora la
   jerarquia es Altura (principal) -> Edad -> Grupo -> Peso, cada linea
   SOLO SI EXISTE (igual regla que ya regia para grupo/peso: nunca se
   rellena con guion ni "Sin especificar").
2. Se reordena ISOFIX / 360º / Orientacion -> ISOFIX / Orientacion / 360º.
3. Se sube el campo "Arnés" de la ficha tecnica a una columna nueva y
   visible en la tabla principal, titulada "Puntos de anclaje" (asi lo
   pidio el usuario). Al subir a la fila principal, se retira de la
   ficha tecnica para no duplicarlo -- mismo criterio que ya se aplico
   en su dia con Giratoria 360º (ver build_comparativa_prototipo_v3.py).
...
Uso:
    python tools/build_comparativa_prototipo_v8.py
"""
...
OUT = "prototipo-comparativa-sillas-v8.html"
...
DETAIL_FIELDS_PRINCIPALES = [
    ...
    # "Arnés" ya no va aqui -- subio a columna visible "Puntos de anclaje"
    # en la tabla principal (v8), no se duplica en la ficha tecnica.
]
...
def build_row(p, idx):
    ...
    <tr class="main-row" data-target="{panel_id}">
      ...
      {celda_principal(arnes, "col-anclaje")}
      <td class="col-action">
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v8 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v8 — <b>no es la web real</b> — misma fuente que v4-v7: tools/output/auditoria_30_candidatos.json (dataset maestro consolidado 13-ago-2026, con la corrección de arnés de Nania Bogota aplicada el 14-ago-2026)</div>
<p class="proto-note">
  Cambios sobre v7: <b>Edad</b> se incorpora dentro de "Rango de uso", justo debajo de la altura (Altura → Edad → Grupo → Peso, cada línea solo si el dato existe). Se reordena a <b>ISOFIX → Orientación → 360º</b>. Se añade la columna <b>"Puntos de anclaje"</b> (campo <code>arnés</code>, antes solo visible en la ficha técnica — al subir a la tabla principal se retira de la ficha para no duplicarlo, mismo criterio que ya se aplicó con 360º en v3).
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v8.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v8] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}}`);
