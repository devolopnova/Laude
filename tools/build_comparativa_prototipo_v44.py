#!/usr/bin/env python3
"""
build_comparativa_prototipo_v44.py

Evolucion sobre v43 (ver tools/build_comparativa_prototipo_v43.py) --
implementa UNICAMENTE los 2 ajustes de la matriz movil acordados con el
usuario (18-ago-2026), EXCLUSIVOS de movil:

  1. Las 2 ultimas filas cambian de orden: Arnés antes que 360º (antes
     era al reves, 360º quedaba penultima). Cambio unico en la lista
     MOBILE_FEATURE_ROWS -- el orden de columnas de la tabla de PC
     (COLUMN_HEADERS) no se toca.

  2. "—" (GUION) pasa a mostrarse como "No especificado" en la matriz
     movil, en las filas Normativa/Orientación/Arnés/ISOFIX/360º (las
     que actualmente mostraban ese simbolo). Dos funciones NUEVAS,
     exclusivas de movil, para no tocar las compartidas con PC:
       - _mc_valor_compacto(): ya existia, ahora convierte GUION en
         "No especificado" antes de devolver el <span> (Normativa/
         Orientación/Arnés). Sigue sin usarse en ningun sitio de PC.
       - _mc_valor_badge(): NUEVA, copia de valor_badge() (que NO se
         toca -- ISOFIX/360º de la tabla de PC siguen mostrando
         exactamente lo mismo que antes) pero con "No especificado" en
         vez de "Sin especificar"/"—". Usada solo para ISOFIX/360º de
         la matriz movil.
     Edad/Altura/Peso/Grupo (las 4 filas de v43) NO se tocan: seguian
     mostrando la celda completamente vacia cuando no hay dato (esa fue
     una regla EXPLICITA distinta pedida por el usuario en el ajuste
     anterior) y esta peticion solo habla del simbolo "—", que esas 4
     filas nunca usaban -- no se reinterpreta esa regla anterior.
     La ficha tecnica movil (#mc-ficha-panel) tampoco se toca: sus
     campos ya mostraban "Sin especificar" completo, no "—", y no
     aparecen en el ejemplo de esta peticion.

No se toca el diseño visual, la tabla de PC, el dataset, las tarjetas
VS superiores, la ficha tecnica, los buscadores/selector de sillitas,
los Slots ni el boton de comparar.

Uso:
    python tools/build_comparativa_prototipo_v44.py
"""
...
OUT = "prototipo-comparativa-sillas-v44.html"
...
<title>PROTOTIPO v44 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v44 — <b>no es la web real</b> — misma fuente que v4-v43: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajustes en la matriz móvil (18-ago-2026): <b>Arnés</b> pasa antes que <b>360º</b> (ahora la última fila), y el símbolo "—" se muestra como <b>"No especificado"</b> en Normativa/Orientación/Arnés/ISOFIX/360º — solo en móvil, la tabla PC sigue mostrando "—" igual que siempre. Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v44.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
