#!/usr/bin/env python3
"""
build_comparativa_prototipo_v11.py

Evolucion sobre v10 (ver tools/build_comparativa_prototipo_v10.py) --
MISMA fuente de datos, MISMO diseno base. Un cambio: se anade la columna
"Valoracion" a la tabla principal, entre Precio y Normativa (⭐
puntuacion arriba, nº de valoraciones abajo, mas pequeno -- ambos datos
proceden exclusivamente del dataset). La ficha tecnica NO se toca (sigue
mostrando tambien la valoracion, tal como pidio el usuario -- aqui no se
aplica el criterio de "no duplicar" porque se pidio explicitamente dejar
la ficha intacta).

Para hacer sitio sin scroll horizontal ni perder legibilidad en Producto
o Rango de uso, se recortan ligeramente Normativa/ISOFIX/Orientacion/
360º/Puntos anclaje/Ficha tecnica, y el texto de "Puntos anclaje" en la
tabla principal se compacta un poco mas ("Arnés de 5 puntos" -> "Arnés 5
puntos", "Cinturón del vehículo" -> "Cinturón vehículo") -- solo
presentacion, el dato real no cambia.
...
Uso:
    python tools/build_comparativa_prototipo_v11.py
"""
...
OUT = "prototipo-comparativa-sillas-v11.html"
...
# Nueva columna "Valoracion" (estrecha) anadida entre Precio y Normativa.
# Producto y Rango de uso mantienen su ancho (no sacrificarlos, pedido
# explicito del usuario); el espacio se recupera recortando ligeramente
# Normativa/ISOFIX/Orientacion/360º/Puntos anclaje/Ficha tecnica, mas el
# texto de Puntos anclaje compactado (ver arnes_compacto_tabla).
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v11 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v11 — <b>no es la web real</b> — misma fuente que v4-v10: tools/output/auditoria_30_candidatos.json (dataset maestro consolidado 13-ago-2026, con la corrección de arnés de Nania Bogota aplicada el 14-ago-2026)</div>
<p class="proto-note">
  Cambio sobre v10: nueva columna <b>Valoración</b> entre Precio y Normativa (⭐ puntuación + nº de valoraciones, ambos del dataset). La ficha técnica no se toca (sigue mostrando también la valoración). Para hacer sitio sin scroll horizontal, se recortan ligeramente Normativa/ISOFIX/Orientación/360º/Puntos anclaje/Ficha técnica, y el texto de "Puntos anclaje" en la tabla se compacta un poco más ("Arnés de 5 puntos" → "Arnés 5 puntos") — solo presentación, el dato no cambia.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v11.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v11] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}}`);
