#!/usr/bin/env python3
"""
build_comparativa_prototipo_v55.py

Evolucion sobre v54 (ver tools/build_comparativa_prototipo_v54.py) --
implementa UNICAMENTE el ajuste de color del boton "Ficha técnica"
movil acordado con el usuario (18-ago-2026).

Cambio (unico, CSS): `.mc-ficha-toggle` (el texto "FICHA TÉCNICA")
pasa de `color:var(--ink)` a `color:var(--accent-deep)` -- la MISMA
variable de naranja de marca que ya usan `.mc-product-brand`,
`.mc-product-price` y `.btn-amazon-row`, sin crear ningun color nuevo.
`.mc-ficha-toggle-icon` (la flecha) YA usaba `var(--accent-deep)` desde
que se creo -- no necesitaba cambio.

Fondo (var(--card), blanco), borde, sombra, tamaño de fuente, padding,
radio de esquinas y el funcionamiento del acordeon (setupMobileFicha(),
sin tocar) se mantienen exactamente igual. No se toca ninguna otra
parte del comparador movil ni la tabla de PC -- ajuste exclusivamente
de color.

Uso:
    python tools/build_comparativa_prototipo_v55.py
"""
...
OUT = "prototipo-comparativa-sillas-v55.html"
...
<title>PROTOTIPO v55 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v55 — <b>no es la web real</b> — misma fuente que v4-v54: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión)</div>
<p class="proto-note">
  Ajuste de color (18-ago-2026): el texto <b>"FICHA TÉCNICA"</b> (móvil) pasa del gris de tinta al mismo naranja de marca que ya usan CYBEX/MAXI-COSI, el precio y "Ver en Amazon →" — la flecha ya lo usaba. Fondo, borde, sombra, tamaño y funcionamiento del acordeón sin cambios. Redimensiona a &lt;768px para verlo.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v55.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni la web real.</p>
