#!/usr/bin/env python3
"""
build_comparativa_prototipo_v13.py

Evolucion sobre v12 (ver tools/build_comparativa_prototipo_v12.py) --
implementa UNICAMENTE el punto 5 acordado con el usuario (16-ago-2026):
boton "Ver en Amazon" visible en la fila principal, junto a "Ficha
tecnica".

  1. La columna de acciones (antes sin cabecera, solo el boton de ficha
     tecnica) se etiqueta "Acciones" y pasa a contener DOS acciones:
     "Ver en Amazon →" (CTA principal, boton solido con --accent-deep)
     y "Ficha técnica ▾" (accion secundaria, boton outline, estilo ya
     existente sin cambios). No se crea ninguna columna nueva -- es la
     misma columna que ya existia.
  2. El enlace de "Ver en Amazon" reutiliza EXACTAMENTE la misma
     construccion de URL que ya se usaba dentro de la ficha tecnica en
     v11/v12 (https://www.amazon.es/dp/<ASIN>, con el ASIN tal cual esta
     en el dataset) -- no se inventa una URL nueva, no se toca ningun
     ASIN, no se usa busqueda generica de Amazon. Abre en pestaña nueva
     (target="_blank", mismo rel="nofollow sponsored noopener" que ya se
     usaba).
  3. La ficha tecnica desplegable NO se toca: mismo contenido, mismo
     comportamiento de apertura/cierre, incluye su propio boton "Ver en
     Amazon" al final tal cual ya existia en v11/v12 (no se retira, no
     se duplica logica nueva alli).

No se modifica ningun dato del dataset, ningun JSON, ni las columnas de
datos (Producto/Precio/Valoracion/Normativa/Rango de uso/ISOFIX/
Orientacion/360º/Arnés) ni la logica de "Sin especificar". No se toca
amazon_import.py ni ningun archivo de produccion. No se implementa
ordenacion, filtros, ni rediseño estetico general -- eso queda para
puntos posteriores.

Uso:
    python tools/build_comparativa_prototipo_v13.py
"""
...
OUT = "prototipo-comparativa-sillas-v13.html"
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v13 — Comparativa sillitas de coche</title>
...
<div class="proto-banner">⚠ Prototipo v13 — <b>no es la web real</b> — misma fuente que v4-v12: tools/output/auditoria_30_candidatos.json (sin cambios de dato en esta versión, solo interfaz)</div>
<p class="proto-note">
  Cambio sobre v12 (punto 5, 16-ago-2026): la columna de acciones (antes solo "Ficha técnica") pasa a tener dos acciones — <b>"Ver en Amazon →"</b> (CTA principal, botón sólido) y "Ficha técnica ▾" (acción secundaria, outline), ambas dentro de la misma columna, sin crear ninguna columna nueva. El enlace reutiliza exactamente la misma URL que ya se construía dentro de la ficha técnica (amazon.es/dp/&lt;ASIN&gt;), sin inventar URLs nuevas. Ninguna otra columna, dato o lógica cambia respecto a v12.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v13.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v13] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}} | botones Ver en Amazon (fila): ${{amazonRowBtns}}`);
