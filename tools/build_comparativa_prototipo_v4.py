#!/usr/bin/env python3
"""
build_comparativa_prototipo_v4.py

Prototipo v4 de la comparativa de sillitas de coche -- PARA PC. A
diferencia de v1-v3 (que leian tools/output/lote1_final.json, 15
productos con clasificacion R44 heredada de la fase antigua), este
prototipo lee EXCLUSIVAMENTE el dataset maestro consolidado el
13-ago-2026: tools/output/auditoria_30_candidatos.json (30 productos,
schema plano, ya auditado y validado -- ver informe de consolidacion en
la conversacion / MEMORY del proyecto).

No re-extrae Amazon/fabricante, no re-audita nada, no reinterpreta
valores, no toca amazon_import.py ni ningun archivo de produccion. Los
campos "Revisar" se muestran tal cual (no se resuelven aqui). Los campos
"Sin especificar" se muestran con tratamiento visual coherente en la
tabla principal, y se OCULTAN por completo dentro de la ficha tecnica
(OPCION A, ver HIDE_UNSPECIFIED_IN_DETAIL mas abajo -- facil de cambiar
a OPCION B en el futuro).

Uso:
    python tools/build_comparativa_prototipo_v4.py
"""
...
OUT = "prototipo-comparativa-sillas-v4.html"

SIN_ESPECIFICAR = "Sin especificar"

# OPCION A (elegida provisionalmente): oculta por completo cualquier
# campo secundario de la ficha tecnica cuyo valor sea "Sin especificar".
# Cambiar a False para volver a la OPCION B (mostrar "Sin especificar")
# sin tocar nada mas del script.
...
# tecnica. "cualquier otro campo secundario confirmado que exista en el
# dataset" (instruccion del usuario) -> se incluye tambien tipo_instalacion,
# que no vive en la tabla principal y tiene cobertura del 100%.
DETAIL_FIELDS = [
...
def valor_badge(valor, kind):
    """ISOFIX / 360º: devuelve el marcado del check compacto + un
    title (tooltip) con el matiz completo cuando el texto es mas largo
    que un simple Si/No."""
    v = (valor or "").strip()
    if es_sin_especificar(v):
        return f'<span class="chk chk--unk" title="{SIN_ESPECIFICAR}">{SIN_ESPECIFICAR}</span>'
    ...
    # Revisar u otro estado no booleano puro
    return f'<span class="chk chk--unk" title="{html_lib.escape(v)}">{html_lib.escape(v)}</span>'


def texto_compacto(valor):
    """Etiqueta corta + matiz completo aparte (para title/tooltip). Un
    valor 'Sin especificar (nota larga...)' se reduce a 'Sin especificar'
    en la celda. El recorte visual real de textos largos lo hace el CSS
    (text-overflow:ellipsis con max-width fijo), no el recuento de
    caracteres aqui -- este helper solo decide la etiqueta y si hace
    falta title/tooltip con el texto integro."""
    v = (valor or SIN_ESPECIFICAR).strip()
    if es_sin_especificar(v):
        return SIN_ESPECIFICAR, (v if v != SIN_ESPECIFICAR else "")
    return v, v


def celda_principal(valor):
    texto, matiz = texto_compacto(valor)
    cls = "cell-unspecified" if texto == SIN_ESPECIFICAR else ("cell-revisar" if es_revisar(texto) else "")
...
def detail_item(label, valor):
    ...
    # El texto de "Revisar" en el dataset puede llevar una nota larga de
    # auditoria -- en la ficha se muestra solo la etiqueta "Revisar" y el
    # detalle completo va en el title (tooltip), para no saturar la UI.
    if es_revisar(v):
        texto = "Revisar"
...
def build_row(p, idx):
    ...
    grupo = p.get("grupo") or SIN_ESPECIFICAR
    peso_recomendado = p.get("peso_recomendado") or SIN_ESPECIFICAR
    altura = p.get("altura") or SIN_ESPECIFICAR
    normativa = p.get("normativa") or SIN_ESPECIFICAR
    orientacion = p.get("orientacion") or SIN_ESPECIFICAR
    isofix = p.get("isofix") or SIN_ESPECIFICAR
    giro_360 = p.get("giro_360") or SIN_ESPECIFICAR

    precio_html = (
        f'<span class="price-current">{precio}</span>'
        if precio else f'<span class="cell-unspecified">{SIN_ESPECIFICAR}</span>'
    )

    grupo_txt, grupo_matiz = texto_compacto(grupo)
    peso_txt, peso_matiz = texto_compacto(peso_recomendado)
    grupo_cls = "cell-unspecified" if grupo_txt == SIN_ESPECIFICAR else ""
    peso_cls = "cell-unspecified" if peso_txt == SIN_ESPECIFICAR else ""
    ...
    for label, key in DETAIL_FIELDS:
        detail_parts.append(detail_item(label, p.get(key)))
    detail_items = "\n".join(d for d in detail_parts if d)
    ...
    <tr class="main-row" data-target="{panel_id}">
      ...
      {celda_principal(normativa)}
      <td class="col-grupo">{grupo_peso_html}</td>
      {celda_principal(altura)}
      <td class="col-isofix">{valor_badge(isofix, "isofix")}</td>
      <td class="col-360">{valor_badge(giro_360, "360")}</td>
      {celda_principal(orientacion)}
      <td class="col-action">
...
# Anchos fijos (px) en el mismo orden que COLUMN_HEADERS -- ver nota CSS
# de table-layout:fixed. Suman ~1160px, con margen para no necesitar
# scroll horizontal en un escritorio normal (>=1366px).
...
<head>
<meta charset="UTF-8">
<title>PROTOTIPO v4 — Comparativa sillitas de coche</title>
...
/* Anchos fijos via colgroup (mas abajo) para que la tabla nunca supere
   ~1320px de ancho total y quepa sin scroll horizontal en un escritorio
   normal (1366px+). Los textos largos se recortan con ellipsis y el
   texto integro va siempre en el title (tooltip). */
...
.cell-unspecified{{color:#9C9A93;font-weight:400;font-style:italic;font-size:12.5px;}}
...
.chk--no{{color:#B5471F;}}
.chk--unk{{color:#9C9A93;font-weight:400;font-style:italic;font-size:11.5px;}}
...
<div class="proto-banner">⚠ Prototipo v4 — <b>no es la web real</b> — fuente única: tools/output/auditoria_30_candidatos.json (dataset maestro consolidado 13-ago-2026)</div>
<p class="proto-note">
  Lee directamente el dataset auditado, sin re-extraer ni reinterpretar. Los campos <code>Sin especificar</code> se muestran en cursiva gris en la tabla principal (nunca como guion) y se ocultan por completo dentro de la ficha técnica (Opción A — configurable con <code>HIDE_UNSPECIFIED_IN_DETAIL</code> en el script). Los campos <code>Revisar</code> se muestran tal cual, sin resolver — pasa el ratón por encima para ver la nota completa de la contradicción.
</p>
...
<p class="foot-note">Prototipo generado por tools/build_comparativa_prototipo_v4.py a partir de tools/output/auditoria_30_candidatos.json — no modifica ningún archivo de producción ni el dataset auditado.</p>
...
(function selfCheck() {{
  ...
  console.log(`[prototipo v4] filas de producto: ${{rows}} | paneles de ficha: ${{panels}} | botones toggle: ${{toggles}}`);
